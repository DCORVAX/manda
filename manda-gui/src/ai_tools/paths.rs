...
//! `manda-gui/AGENTS.md`). Keeping it isolated also makes the security check
...
fn blocked_locations() -> Vec<PathBuf> {
    ...
    if let Ok(home) = home() {
        for rel in [
            ...
            ".gnupg",
            ".config/gh",
            ".config/manda/assistant.toml",
            ".config/manda/secrets",
            ".docker/config.json",
            ".git-credentials",
            ".netrc",
            ".npmrc",
...
mod tests {
    ...
    fn reject_if_sensitive_blocks_assistant_config() {
        let home = std::env::var("HOME").expect("HOME not set");
        let assistant_config = PathBuf::from(&home).join(".config/manda/assistant.toml");
