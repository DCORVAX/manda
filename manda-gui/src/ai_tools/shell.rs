...
fn shell_exec_wrapper(command: &str, cwd_tmp_path: &Path, shell: &str) -> String {
    ...
    if is_fish {
        format!(
            "{}; set __manda_rc $status; printf '%s' (pwd) > {}; exit $__manda_rc",
            command,
            cwd_tmp_path.display()
        )
    } else {
        format!(
            "{}; __manda_rc=$?; printf '%s' \"$(pwd)\" > {}; exit $__manda_rc",
            command,
            cwd_tmp_path.display()
        )
    }
...
pub(super) fn exec_shell_exec(
    ...
    let ts = std::time::SystemTime::now()
        ...
        .unwrap_or(0);
    let cwd_tmp_path =
        std::env::temp_dir().join(format!("manda_cwd_{}_{}.txt", std::process::id(), ts));
    ...
    stdout_lines.retain(|l| !l.starts_with("__MANDA_CWD__:"));
...
mod tests {
    ...
    fn shell_exec_wrapper_uses_fish_syntax() {
        let wrapped =
            shell_exec_wrapper("false", Path::new("/tmp/manda-cwd"), "/usr/local/bin/fish");

        assert_eq!(
            wrapped,
            "false; set __manda_rc $status; printf '%s' (pwd) > /tmp/manda-cwd; exit $__manda_rc"
        );
    }
    ...
    fn shell_exec_wrapper_keeps_posix_syntax_for_zsh_and_bash() {
        for shell in ["/bin/bash", "/bin/zsh"] {
            let wrapped = shell_exec_wrapper("false", Path::new("/tmp/manda-cwd"), shell);

            assert_eq!(
                wrapped,
                "false; __manda_rc=$?; printf '%s' \"$(pwd)\" > /tmp/manda-cwd; exit $__manda_rc"
            );
        }
    }
}
