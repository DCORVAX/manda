//! AI client for MANDA's built-in chat overlay.
//!
//! Reads API config from `~/.config/manda/assistant.toml` and provides
...
impl ApiMode {
    fn from_config(value: Option<&str>) -> Self {
        match value.unwrap_or("chat_completions") {
            ...
            other => {
                // Same tolerant policy as the `manda ai` TUI, which coerces
...
impl AssistantConfig {
    ...
    pub fn load() -> Result<Self> {
        ...
        if auth_type == "gemini_key" {
            anyhow::bail!(
                "Gemini provider was removed in V0.10.0. Open `manda ai` and \
        ...
        // MANDA env-first key resolution: when `api_key` is not stored in
        // assistant.toml, fall back to the matched provider's environment
        // variable (e.g. NVIDIA_API_KEY, GEMINI_API_KEY, ...) so secrets can
        // stay out of the config file.
        if api_key.is_empty() {
            if let Some(preset) = manda_ai_utils::providers::detect_provider_for_base_url(&base_url)
            {
                if let Ok(env_key) = std::env::var(preset.api_key_env) {
                    api_key = env_key;
                }
            }
        }
...
impl ApiMessage {
    ...
    pub fn responses_output_item(item: serde_json::Value) -> Self {
        Self(serde_json::json!({ "manda_responses_output_item": item }))
    }
...
fn translate_responses_messages(messages: &[ApiMessage]) -> (String, Vec<serde_json::Value>) {
    ...
    for ApiMessage(message) in messages {
        if let Some(item) = message.get("manda_responses_output_item") {
...
fn format_response_citations(citations: &[(String, String)]) -> String {
    ...
    for (title, url) in citations {
        // MANDA's compact Markdown renderer intentionally drops link targets,
...
///
/// Single source of truth for provider naming. The `manda` binary used to
...
fn detect_provider_with_auth(base_url: &str, auth_type: &str) -> &'static str {
    // MANDA's five built-in OpenAI-compatible provider presets come first so
    // the GUI shows a friendly name instead of "Custom".
    if let Some(preset) = manda_ai_utils::providers::detect_provider_for_base_url(base_url) {
        return preset.label;
    }
...
mod tests {
    ...
    #[test]
    fn detects_manda_provider_presets() {
        // MANDA's five built-in OpenAI-compatible presets map to friendly labels.
        assert_eq!(
            detect_provider_with_auth("https://integrate.api.nvidia.com/v1", "api_key"),
            "NVIDIA NIM"
        );
        assert_eq!(
            detect_provider_with_auth(
        ...
        assert_eq!(
            detect_provider_with_auth("https://openrouter.ai/api/v1", "api_key"),
            "OpenRouter"
        );
        assert_eq!(
            detect_provider_with_auth("https://api.groq.com/openai/v1", "api_key"),
            "Groq"
        );
        assert_eq!(
            detect_provider_with_auth("https://api.cerebras.ai/v1", "api_key"),
            "Cerebras"
        );
        // Trailing slashes do not break preset detection.
        assert_eq!(
            detect_provider_with_auth("https://api.cerebras.ai/v1/", "api_key"),
            "Cerebras"
        );
    ...
    fn custom_headers_replace_existing_user_agent_without_dropping_auth() {
        let config = AssistantConfig {
            ...
            custom_headers: vec![
                ("User-Agent".to_string(), "MANDA-Test".to_string()),
                ("X-Customer-ID".to_string(), "acme".to_string()),
            ],
            provider: "Custom".to_string(),
        ...
        assert_eq!(user_agents[0], "MANDA-Test");
        assert_eq!(
