...
async fn async_run_terminal_gui(
    ...
    std::env::set_var("MANDA_UNIX_SOCKET", unix_socket_path.clone());
                ...
                Ok(client) => {
                    ...
                    let res = block_on(executor.run(async move {
                        ...
                        if vers.config_file_path
                            != std::env::var_os("MANDA_CONFIG_FILE").map(Into::into)
                        {
                            *self = Publish::NoConnectNoPublish;
                            anyhow::bail!(
                    ...
                    match res {
                        Ok(res) => {
                            log::debug!(
                             ...
                             Use manda start --always-new-process if you do not want this behavior. \
                             Result={:?}",
                                res
                            );
                            Ok(true)
...
fn run_terminal_gui(opts: StartCommand, default_domain_name: Option<String>) -> anyhow::Result<()> {
    ...
    // First, let's see if we can ask an already running MANDA instance to do this.
    ...
    if let Err(err) = manda_gui_lib::inline_ai_control::initialize_capability() {
...
fn fatal_toast_notification(title: &str, message: &str) {
    let should_show = if cfg!(debug_assertions) {
        std::env::var_os("MANDA_DEV_FATAL_TOAST").is_some()
    } else {
        true
    };
...
fn notify_on_panic() {
    ...
    std::panic::set_hook(Box::new(move |info| {
        if let Some(s) = info.payload().downcast_ref::<&str>() {
            fatal_toast_notification("MANDA panic", s);
...
fn terminate_with_error_message(err: &str) -> ! {
    ...
    fatal_toast_notification("MANDA Error", &err);
    std::process::exit(1);
}
...
fn run() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    {
        unsafe {
            ::windows::Win32::UI::Shell::SetCurrentProcessExplicitAppUserModelID(
                ::windows::core::PCWSTR(wide_string("sh.manda.MANDA").as_ptr()),
            )
            .unwrap();
        }
    }
    ...
    let sub = match opts.cmd.as_ref().cloned() {
        ...
        None => {
            ...
            let mut argv = vec!["manda-gui".to_string()];
