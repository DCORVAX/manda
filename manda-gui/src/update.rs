...
pub struct StagedUpdateInfo {
    ...
    pub staged_at: u64,
    pub verified: bool,
    /// Path to the extracted Manda.app inside the staged_update directory.
    pub app_path: String,
}
...
///
/// Two MANDA processes started at once would otherwise race on
    ...
    fn try_acquire() -> anyhow::Result<Self> {
        ...
        if rc != 0 {
            ...
            if matches!(err.raw_os_error(), Some(libc::EWOULDBLOCK)) {
                anyhow::bail!("another MANDA process is already staging an update");
            }
            return Err(anyhow!(
...
fn curl_get_release_json(url: &str, proxy: &Option<String>) -> anyhow::Result<Release> {
    ...
    cmd.arg("--fail")
        ...
        .arg("15")
        .arg("--user-agent")
        .arg(format!("manda/{}", wezterm_version()))
        .arg(url);
...
fn download_and_stage_update(
    ...
    // Hold an exclusive lock for the whole staging operation so two MANDA
    ...
    let new_app_path = find_manda_app(&extracted_dir)
        .ok_or_else(|| anyhow!("update package does not contain Manda.app"))?;
...
fn update_checker() {
    ...
    let force_ui = std::env::var_os("MANDA_ALWAYS_SHOW_UPDATE_UI").is_some();
...
// ---------------------------------------------------------------------------
// Helper script for restart-to-update (duplicated from manda/src/update.rs
// because manda-gui does not depend on the manda crate).
// ---------------------------------------------------------------------------

/// Resolve the installed Manda.app path, matching the logic in the CLI crate.
pub fn resolve_target_app_path() -> anyhow::Result<PathBuf> {
    if let Some(path) = std::env::var_os("MANDA_UPDATE_TARGET_APP") {
        ...
        anyhow::bail!("MANDA_UPDATE_TARGET_APP must point to Manda.app");
    }
    ...
    anyhow::bail!("cannot locate installed Manda.app")
}

pub fn write_update_helper_script(script_path: &Path) -> anyhow::Result<()> {
    // This is the same helper script as manda/src/update.rs::write_helper_script.
...
pub fn spawn_update_helper(
    ...
    // Validate paths end with Manda.app.
    if !target_app.ends_with("Manda.app") {
        anyhow::bail!(
            "target_app must end with Manda.app: {}",
            target_app.display()
        );
    }
    if !new_app.ends_with("Manda.app") {
        anyhow::bail!("new_app must end with Manda.app: {}", new_app.display());
    }
...
mod tests {
    ...
    /// Run with: cargo nextest run -p manda-gui staged_update_e2e -- --ignored
    #[test]
    #[ignore]
    fn staged_update_e2e() {
        ...
        // Find Manda.app
        let app_path = find_manda_app(&extracted).expect("should find Manda.app");
