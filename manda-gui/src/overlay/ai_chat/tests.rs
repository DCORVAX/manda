...
mod markdown_tests {
    ...
    fn shell_exec_read_only_commands_skip_approval() {
        for command in [
            ...
            "gh pr diff 456",
            "gh pr checks 456",
            "gh repo view WILFREDY-X/manda",
            "gh release list",
            "gh release view v0.10.0",
            "gh workflow list",
            "gh run list",
            "gh search issues manda",
            "gh search prs --repo WILFREDY-X/manda",
            "gh auth status",
            "gh status",
            "gh api repos/WILFREDY-X/manda",
            "gh api -X GET repos/WILFREDY-X/manda",
            "gh api --method=GET repos/WILFREDY-X/manda",
            ...
            "curl https://api.github.com/repos/WILFREDY-X/manda",
            "curl -s https://api.github.com/repos/WILFREDY-X/manda",
            "curl -sL https://api.github.com/repos/WILFREDY-X/manda/issues",
            "curl -X GET https://api.github.com/repos/WILFREDY-X/manda",
            "curl --request=GET https://api.github.com/repos/WILFREDY-X/manda",
            ...
            "cd /tmp",
            "cd ~/www/MANDA",
            "cd ~/www/MANDA && grep -irA 5 -B 5 correction manda-gui/src",
            "cd /tmp && ls -la",
            "cd ~/www/MANDA && pwd && ls",
            ...
            "ls -la ~/www/manda 2>/dev/null",
            "ls 2> /dev/null",
            "cat foo 2>&1 | grep bar",
            "ls -la ~/www/manda 2>/dev/null || echo \"Not found\"",
        ] {
            assert!(
    ...
    fn shell_exec_dangerous_commands_require_approval() {
        for command in [
            ...
            "gh auth login",
            "gh auth logout",
            "gh api -X POST repos/WILFREDY-X/manda/issues",
            "gh api --method POST repos/WILFREDY-X/manda/issues",
            "gh api repos/WILFREDY-X/manda/issues -F title=hi",
