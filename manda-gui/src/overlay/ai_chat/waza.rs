//! Built-in Waza skill snapshots for MANDA AI chat.
//!
//! These are compact, MANDA-compatible mode instructions. They intentionally do
...
You are running a built-in Waza skill inside MANDA AI chat. This instruction is \
active for the current user turn only.

Adapt the workflow to MANDA's available tools. Use grep_search for code search, \
fs_read/fs_list for file inspection, shell_exec for verification, web_fetch for \
URLs, web_search/read_url only when configured, and existing MANDA approval gates \
...
analysis or summary. If the request is only to save content and MANDA lacks a \
