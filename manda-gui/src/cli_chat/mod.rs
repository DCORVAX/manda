...
pub fn run(args: CliArgs) -> anyhow::Result<()> {
    // When running inside a MANDA pane with no extra args, trigger the Cmd+L
    ...
    let inside_manda = std::env::var_os("MANDA_UNIX_SOCKET").is_some();
    if inside_manda && args.prompt.is_none() && !args.new && args.resume.is_none() {
        ...
        print!("\x1b]1337;SetUserVar=manda_open_ai_chat={}\x07", encoded);
