...
fn is_ai_progress_toast(message: &str) -> bool {
    matches!(
        message,
        "MANDA Assistant analyzing command" | "MANDA generating command"
    )
}
...
mod tests {
    ...
    fn only_ai_progress_toasts_are_clearable() {
        assert!(is_ai_progress_toast("MANDA Assistant analyzing command"));
        assert!(is_ai_progress_toast("MANDA generating command"));
        assert!(!is_ai_progress_toast("MANDA Assistant is not configured"));
