...
// MANDA palette visual defaults. Used only when the user keeps the stock
// command_palette_* colors, so custom config still takes precedence.
const MANDA_BG: LinearRgba = LinearRgba::with_components(0.082, 0.078, 0.106, 0.985);
const MANDA_FG: LinearRgba = LinearRgba::with_components(0.929, 0.925, 0.933, 1.0);
const MANDA_ACCENT: LinearRgba = LinearRgba::with_components(0.635, 0.467, 1.0, 1.0);
const MANDA_SELECTION_BG: LinearRgba = LinearRgba::with_components(0.161, 0.149, 0.235, 1.0);
const MANDA_DIM_FG: LinearRgba = LinearRgba::with_components(0.420, 0.420, 0.420, 1.0);
const MANDA_SEPARATOR: LinearRgba = LinearRgba::with_components(0.2, 0.18, 0.28, 0.34);
    ...
    fn palette_theme(term_window: &TermWindow) -> PaletteTheme {
        if Self::using_default_palette_colors(term_window) {
            return PaletteTheme {
                ...
                accent: MANDA_ACCENT,
                dim_fg: MANDA_DIM_FG,
                separator: MANDA_SEPARATOR,
                selection_bg: MANDA_SELECTION_BG,
            };
        }
