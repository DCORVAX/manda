...
fn normalize_bell_notification_source(value: &str) -> Option<String> {
    ...
    if first_line.is_empty()
        || first_line.eq_ignore_ascii_case("manda")
...
fn lookup_manda_toast(event_name: &str) -> Option<&'static str> {
    const MANDA_TOAST_MAP: &[(&str, &str)] = &[
        ("manda-toast-try-lazygit", "Try Lazygit: Cmd+Shift+G"),
        ("manda-toast-lazygit-no-pane", "Lazygit: No active pane"),
        (
            "manda-toast-lazygit-no-cwd",
            "Lazygit: Cannot detect current directory",
        ),
        (
            "manda-toast-lazygit-not-git",
            "Lazygit: Not a git repository",
        ),
        (
            "manda-toast-lazygit-missing",
            "Lazygit not found. Run manda init",
        ),
        (
            "manda-toast-lazygit-dispatch-failed",
            "Lazygit: Dispatch failed",
        ),
        ("manda-toast-yazi-no-pane", "Yazi: No active pane"),
        ("manda-toast-yazi-missing", "Yazi not found. Run manda init"),
        ("manda-toast-yazi-dispatch-failed", "Yazi: Dispatch failed"),
    ];
    MANDA_TOAST_MAP
        .iter()
...
fn lookup_ai_toast(event_name: &str) -> Option<&'static str> {
    const AI_TOAST_MAP: &[(&str, &str)] = &[
        (
            "manda-toast-ai-ready",
            "MANDA Assistant suggestion ready. Press Cmd+Shift+E",
        ),
        ("manda-toast-ai-unavailable", "MANDA Assistant unavailable"),
        (
            "manda-toast-ai-missing-key",
            "Run manda ai to set up MANDA Assistant.",
        ),
        ("manda-toast-ai-no-pane", "No active pane"),
        ("manda-toast-ai-no-suggestion", "No executable suggestion"),
        ("manda-toast-ai-send-failed", "Failed to apply suggestion"),
        ("manda-toast-ai-info", "MANDA Assistant update"),
    ];
    AI_TOAST_MAP
        .iter()
        .find(|(k, _)| *k == event_name)
...
impl TermWindow {
    ...
    fn should_reload_config_for_user_var(name: &str, _window_contains_pane: bool) -> bool {
        // We used to require `window_contains_pane && name == "MANDA_CONFIG_CHANGED"`
        // to avoid duplicate reloads when multiple windows are open. However, when
        // a user exits `manda config` very quickly (e.g. by hitting ESC), the pane
    ...
    fn created(&mut self, ctx: RenderContext) -> anyhow::Result<()> {
        ...
        match RenderState::new(ctx, &self.fonts, &self.render_metrics, ATLAS_SIZE) {
            Ok(render_state) => {
                log::debug!(
                    "OpenGL initialized! {} MANDA version: {}",
                    render_info,
                        ...
                        let (last_command, reported_program, pane_title, foreground_process) =
                            Mux::get()
                                ...
                                .map(|pane| {
                                    ...
                                    (
                                        user_vars.get("manda_last_cmd").and_then(|value| {
                                            manda_gui_lib::inline_ai_control::decode_control_value(
                                                "manda_last_cmd",
                                                value,
                                            )
                                            .map(str::to_string)
...
impl TermWindow {
    ...
    fn emit_user_var_event(&mut self, pane_id: PaneId, name: String, value: String) {
        ...
        let value = match manda_gui_lib::inline_ai_control::decode_control_value(&name, &value) {
            Some(value) => value.to_string(),
            None => {
                log::warn!("Ignored unauthenticated MANDA control user var {name}");
                return;
            }
        };

        // `k` CLI running inside a MANDA pane signals us to open the AI chat overlay.
        if name == "manda_open_ai_chat" {
                ...
                win.notify(TermWindowNotif::Apply(Box::new(move |term_window| {
                    ...
                    if let Some(pane) = mux.get_pane(pane_id) {
                        let _ = term_window.perform_key_assignment(
                            &pane,
                            &KeyAssignment::EmitEvent("manda-ai-chat".to_string()),
                        );
                    }
                })));
            }
    ...
    pub fn perform_key_assignment(
        ...
        match assignment {
            ...
            ShowDebugOverlay => {
                crate::frontend::run_manda_doctor_in_new_tab();
            }
            ...
            EmitEvent(name) => {
                if let Some(job_id) = name.strip_prefix(crate::inline_ai::EVENT_PREFIX) {
                    ...
                    return Ok(PerformAssignmentResult::Handled);
                } else if name == "manda-ai-chat" {
                    ai_chat::toggle_overlay(self, pane);
                } else if name == "update-manda" || name == "run-manda-update" {
                    crate::frontend::check_for_updates_from_menu();
                } else if name == "restart-to-update" {
                    crate::frontend::confirm_and_apply_update();
                } else if name == "run-manda-cli" {
                    let command = format!("{}\n", crate::frontend::manda_cli_shell_invocation());
                    let result = pane.writer().write_all(command.as_bytes());
                    self.finish_terminal_input(pane, result)?;
                } else if name == "run-manda-ai-config" {
                    let command = format!("{} ai\n", crate::frontend::manda_cli_shell_invocation());
                    let result = pane.writer().write_all(command.as_bytes());
                    self.finish_terminal_input(pane, result)?;
                } else if let Some(msg) = lookup_manda_toast(name) {
                    self.show_toast(msg.to_string());
                } else if name == "manda-toast-ai-analyzing" {
                    let message = "MANDA Assistant analyzing command";
                    self.show_ai_progress_toast(message.to_string(), ai_toast_lifetime_ms(message));
                } else if name == "manda-toast-ai-generating" {
                    let message = "MANDA generating command";
                    self.show_ai_progress_toast(message.to_string(), ai_toast_lifetime_ms(message));
                } else if name == "manda-toast-ai-clear-progress" {
                    self.clear_ai_progress_toast();
                } else if name == "manda-toast-ai-applied" {
                ...
                } else if let Some(payload) = name.strip_prefix("manda-toast-ai-") {
                ...
                } else if name == "open-manda-config" {
                    crate::frontend::open_manda_config();
                } else if name == crate::frontend::SET_DEFAULT_TERMINAL_EVENT {
                    match Connection::get() {
                        Some(conn) => match conn.set_default_terminal() {
                            Ok(()) => {
                                self.show_toast("MANDA is now the default terminal".to_string());
                            }
                            Err(err) => {
                                log::error!("Failed to set MANDA as default terminal: {err:#}");
...
mod tests {
    ...
    fn bell_notification_message_uses_reported_program_before_default_title() {
        assert_eq!(
            bell_notification_message(None, Some("npm run build"), "manda", None),
    ...
    fn bell_notification_message_falls_back_to_process_basename() {
        assert_eq!(
            bell_notification_message(None, None, "manda", Some("/opt/homebrew/bin/git")),
            "Task complete: git"
        );
    }
    ...
    fn bell_notification_message_uses_background_fallback_for_uninformative_values() {
        assert_eq!(
            bell_notification_message(Some("   "), Some("wezterm"), "manda", None),
    ...
    fn bell_notification_message_summarizes_shell_wrapped_command() {
        assert_eq!(
            bell_notification_message(
                Some("zsh -lc cargo check -p manda-gui"),
                None,
                "manda",
                None
            ),
    ...
    fn bell_notification_message_handles_sudo_with_user_flag() {
        assert_eq!(
            bell_notification_message(Some("sudo -u root cargo build"), None, "manda", None),
    ...
    fn bell_notification_message_handles_docker_compose_up() {
        assert_eq!(
            bell_notification_message(Some("docker compose up -d"), None, "manda", None),
