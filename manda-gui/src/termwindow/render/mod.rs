...
pub(crate) fn forces_opaque_manda_tui_background(pane: &Arc<dyn Pane>) -> bool {
    ...
    if executable.as_deref() != Some("manda") {
        return false;
    }
...
pub(crate) fn forces_opaque_manda_tui_window_background(panes: &[PositionedPane]) -> bool {
    panes.len() == 1 && forces_opaque_manda_tui_background(&panes[0].pane)
}
