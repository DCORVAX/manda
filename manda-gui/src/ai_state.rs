...
//!
//! Stores UI state like the last selected model in `~/.config/manda/ai_chat_state.json`.
//! Load once at overlay start; save when the user switches models.
//!
//! # Why two model-list caches?
//!
//! MANDA has two on-disk caches of the assistant's `/models` response that
//! intentionally do *not* share a file:
//!
//! | Surface | Path | TTL | Purpose |
//! |---|---|---|---|
//! | `manda ai` TUI | `~/.cache/manda/assistant_models.json` | 30 min, base_url-keyed | Driving the dropdown picker; refreshed every TUI session so a freshly-added model shows up. |
//! | Cmd+L overlay (here) | `~/.config/manda/ai_chat_state.json` `cached_models` | persistent, no expiry | Avoid a "loading models…" flash on the very next overlay open; merged with `chat_model` and the curated `chat_model_choices` list. |
//!
