...
fn resolve_bundled_manda_bin() -> anyhow::Result<PathBuf> {
    ...
    if let Some(parent) = current_exe.parent() {
        add_candidate(&mut candidates, parent.join("manda"));
    }
    ...
    anyhow::bail!(
        "could not find manda binary; checked: {}",
        candidates
            .iter()
...
pub(crate) fn manda_cli_program_for_spawn() -> String {
    match resolve_bundled_manda_bin() {
        Ok(path) => path.to_string_lossy().into_owned(),
        Err(err) => {
            // Finder-launched apps can have a minimal PATH; fall back only when
            // we cannot resolve the bundled companion binary.
            log::warn!("Falling back to PATH lookup for `manda`: {err:#}");
            "manda".to_string()
        }
    }
}
...
/// via `manda_cli_program_for_spawn` instead.
pub(crate) fn manda_cli_shell_invocation() -> String {
    let manda_bin = manda_cli_program_for_spawn();
    shell_quote_program(&manda_bin)
}
...
pub fn open_manda_config() {
    let manda_bin = manda_cli_program_for_spawn();
    ensure_singleton_window("manda-config", async move || {
        ...
        crate::spawn::spawn_command_internal(
            SpawnCommand {
                ...
                args: Some(vec![manda_bin, "config".to_string()]),
                ..Default::default()
            },
...
// - Menu Check (download)   -> run_manda_update_in_tab(auto_confirm=false) -> CLI prompt
// - Overlay confirmed       -> apply_update_now -> staged restart or tab(auto_confirm=true)
// - CLI direct / brew       -> confirm_apply_update in manda/src/update.rs
...
/// `manda update` interactively so the CLI can prompt before install.
pub fn check_for_updates_from_menu() {
    if crate::update::staged_update_available().is_some() {
        confirm_and_apply_update();
    } else {
        run_manda_update_in_tab(/* auto_confirm */ false);
    }
}

/// Open a terminal tab running `manda update` without pre-confirming install.
/// Used by the menu check path and by staged-update failure fallbacks.
pub fn run_manda_update_from_menu() {
    run_manda_update_in_tab(/* auto_confirm */ false);
}

fn run_manda_update_in_tab(auto_confirm: bool) {
    static UPDATE_RUNNING: AtomicBool = AtomicBool::new(false);
    if UPDATE_RUNNING
        .compare_exchange(false, true, Ordering::SeqCst, Ordering::SeqCst)
        .is_err()
    {
        log::info!("run_manda_update_in_tab: update already running, ignoring");
        return;
    }
    run_manda_subcommand_in_new_tab("update", Some(&UPDATE_RUNNING), auto_confirm);
}
...
pub(crate) fn apply_update_now() {
    if crate::update::staged_update_available().is_some() {
        ...
        run_manda_update_in_tab(/* auto_confirm */ true);
    }
}
...
pub fn restart_to_update() {
    ...
    // scripts race to ditto into the same Manda.app.
    static UPDATE_RUNNING: AtomicBool = AtomicBool::new(false);
    if UPDATE_RUNNING
        .compare_exchange(false, true, Ordering::SeqCst, Ordering::SeqCst)
        .is_err()
...
pub fn run_manda_doctor_in_new_tab() {
    run_manda_subcommand_in_new_tab("doctor", None, false);
}

fn run_manda_subcommand_in_new_tab(
    subcommand: &str,
    ...
    let manda_bin = manda_cli_program_for_spawn();
    let fallback_bin = shlex::try_quote(&manda_bin)
        .map(|q| q.into_owned())
        .unwrap_or_else(|_| manda_bin.clone());
    ...
    let shell_name = Path::new(&shell)
        ...
        .unwrap_or_default();
    // Use login + interactive shell (-lic / -l -i -c) to match the default MANDA
    ...
    // Only set MANDA_UPDATE_AUTO_CONFIRM after the user already confirmed in the
...
pub fn set_default_terminal_with_feedback() {
    ...
    match Connection::get() {
        Some(conn) => match conn.set_default_terminal() {
            Ok(()) => {
                let message = "MANDA is now the default terminal";
                if !show_window_toast(message) {
                    conn.alert("Default Terminal", message);
                }
            }
            Err(err) => {
                let message = format!("Failed to set MANDA as default terminal: {err:#}");
                log::error!("{message}");
            ...
            ApplicationEvent::PerformKeyAssignment(action) => {
                ...
                match action {
                    KeyAssignment::EmitEvent(event)
                        if event == "update-manda" || event == "run-manda-update" =>
                    {
                    ...
                    KeyAssignment::EmitEvent(event) if event == "run-manda-cli" => {
                        let manda_cli = manda_cli_program_for_spawn();
                        spawn_command(
                            &SpawnCommand {
                                args: Some(vec![manda_cli]),
                                ..Default::default()
                            },
                            SpawnWhere::NewWindow,
                        );
                    }
                    KeyAssignment::EmitEvent(event) if event == "open-manda-config" => {
                        open_manda_config();
                    }
            ...
            while let Some(mux_window_id) = mux_windows.next() {
                ...
                if let Err(err) = TermWindow::new_window(mux_window_id).await {
                    ...
                    if err_text.contains("failed to create NSOpenGLPixelFormat") {
                        log::error!(
                            "OpenGL initialization failed. This often means no compatible GPU renderer is available (for example in some VMs). Try setting `front_end = 'WebGpu'` in manda.lua or enabling VM GPU acceleration."
                        );
                    }
                    let mux = Mux::get();
...
mod tests {
    ...
    fn shell_program_with_spaces_round_trips_as_one_token() {
        let program = "/Applications/MANDA Nightly.app/Contents/MacOS/manda";
    ...
    fn user_facing_update_events_route_through_confirm() {
        ...
        assert!(
            ...
            "run-manda-update must go through check_for_updates_from_menu"
        );
        assert!(
        ...
        assert!(
            ...
            "menubar run-manda-update must confirm-or-prompt"
        );
