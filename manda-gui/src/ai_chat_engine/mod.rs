...
//!
//! Used by both the Cmd+L overlay (manda-gui binary) and the `k` standalone CLI.
...
mod tests {
    ...
    fn build_api_messages_replays_persisted_responses_state() {
        ...
        assert!(messages
            .iter()
            .any(|message| { message.0.get("manda_responses_output_item") == Some(&reasoning) }));
        assert!(!messages
            .iter()
    ...
    fn build_system_prompt_concatenates_all_fragments() {
        ...
        assert!(prompt.contains("MANDA AI"), "missing voice fragment");
        assert!(
