...
/// another MANDA process may already be using.
...
mod tests {
    ...
    fn privileged_user_vars_require_the_active_capability() {
        ...
        assert_eq!(
            decode_control_value_with_capability("manda_ai_query", &value, Some("wrong")),
            None
        );
        assert_eq!(
            decode_control_value_with_capability("manda_ai_query", &value, None),
            None
        );
        assert_eq!(
