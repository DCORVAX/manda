...
pub fn get_copilot_token(client: &reqwest::blocking::Client) -> Result<String> {
    let mut auth = load_copilot_auth().ok_or_else(|| {
        anyhow::anyhow!(
            "Copilot: not logged in. Open `manda ai` and select Copilot, then press Enter on \
    ...
    if auth.github_token.trim().is_empty() {
        anyhow::bail!(
            "Copilot: GitHub token missing. Open `manda ai` and authenticate via GitHub."
        );
    }
...
/// ~/.codex/auth.json. Codex writes these after `codex` login; MANDA reads them
