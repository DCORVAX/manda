...
impl CommandDef {
    ...
    pub fn actions_for_palette_only(config: &ConfigHandle) -> Vec<ExpandedCommand> {
        ...
        fn is_high_value_discovery_action(action: &KeyAssignment) -> bool {
            matches!(
                ...
                action,
                EmitEvent(name)
                    if name == "manda-ai-chat"
                        || name == "manda-launch-lazygit"
                        || name == "manda-launch-yazi"
                        || name == "run-manda-ai-config"
            )
        }
    ...
    pub fn recreate_menubar(config: &ConfigHandle) {
        ...
        let manda_perform_key_assignment_sel = sel!(mandaPerformKeyAssignment:);
        ...
        let main_menu = match Menu::get_main_menu() {
            Some(existing) => {
                mark_candidates(
                    ...
                    manda_perform_key_assignment_sel,
                );
        ...
        let mut order: Vec<&'static str> = vec!["MANDA", "Shell", "Edit", "View", "Window", "Help"];
        ...
        fn command_rank_for_menu(title: &str, action: &KeyAssignment) -> usize {
            match title {
                ...
                "Shell" => match action {
                    ...
                    EmitEvent(name) if name == "manda-ai-chat" => 20,
                    EmitEvent(name) if name == "run-manda-ai-config" => 21,
                    EmitEvent(name) if name == "manda-launch-lazygit" => 22,
                    EmitEvent(name) if name == "manda-launch-yazi" => 23,
                    EmitEvent(name) if name == "manda-open-remote-files" => 24,
        ...
        for &title in &order {
            ...
            for cmd in menu_commands {
                ...
                let mut submenu = main_menu.get_or_create_sub_menu(&cmd.menubar[0], |menu| {
                    ...
                    // routing. MANDA owns its own tab/window switcher and
                    // has no help book, so the AppKit-managed children
                    // have no value here.
                    if cmd.menubar[0] == "MANDA" {
                        menu.assign_as_app_menu();

                        let about_item = MenuItem::new_with(
                            &format!("MANDA V{}", config::wezterm_version()),
                            Some(manda_perform_key_assignment_sel),
                            "",
                        );
                        about_item.set_represented_item(RepresentedItem::KeyAssignment(
                            KeyAssignment::EmitEvent("run-manda-cli".to_string()),
                        ));
                        ...
                        check_update.set_represented_item(RepresentedItem::KeyAssignment(
                            KeyAssignment::EmitEvent("run-manda-update".to_string()),
                        ));
...
pub fn derive_command_from_key_assignment(action: &KeyAssignment) -> Option<CommandDef> {
    ...
    Some(match action {
        ...
        HideApplication => CommandDef {
            brief: "Hide MANDA".into(),
            doc: "Hide all MANDA windows".into(),
        ...
        ShowDebugOverlay => CommandDef {
            brief: "MANDA Doctor".into(),
            doc: "Run manda doctor in the current pane".into(),
        ...
        EmitEvent(name) => {
            ...
            } else if name == "manda-ai-apply-last-fix" {
                CommandDef {
                    brief: "Apply Last AI Fix".into(),
                    doc: "Apply the latest MANDA Assistant suggestion for the active pane".into(),
                    keys: vec![(Modifiers::SUPER.union(Modifiers::SHIFT), "e".into())],
                    args: &[ArgType::ActiveWindow],
                    menubar: &["Shell"],
                    icon: None,
                }
            } else if name == "manda-launch-lazygit" {
                CommandDef {
                    brief: "Lazygit".into(),
            ...
            } else if name == "manda-launch-yazi" {
                CommandDef {
            ...
            } else if name == "manda-open-remote-files" {
                CommandDef {
        ...
        QuitApplication => CommandDef {
            brief: "Quit MANDA".into(),
            doc: "Quits MANDA".into(),
...
fn compute_default_actions() -> Vec<KeyAssignment> {
    ...
    let mut actions = vec![
        ...
        SpawnTab(SpawnTabDomain::CurrentPaneDomain),
        SpawnWindow,
        EmitEvent("manda-ai-chat".to_string()),
        EmitEvent("run-manda-ai-config".to_string()),
        EmitEvent("manda-launch-lazygit".to_string()),
        EmitEvent("manda-launch-yazi".to_string()),
        EmitEvent("manda-open-remote-files".to_string()),
        SplitVertical(SpawnCommand {
            domain: SpawnTabDomain::CurrentPaneDomain,
            ..Default::default()
        }),
