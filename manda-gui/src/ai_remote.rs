//! Wires remote AI chat requests (from the iOS app via manda-remote) to the
...
use manda_remote::{set_ai_handler, AiEvent, AiHandler, AiRequest};
...
/// Safe to call at any point after manda-remote is initialised. Returns early
...
pub fn register_if_configured() {
    let config = match AssistantConfig::load() {
        ...
        Err(err) => {
            log::info!(
                "manda-remote AI: not registering remote AI handler ({}). \
                 Configure ~/.config/manda/assistant.toml and restart.",
                err
            );
            return;
        }
    ...
    let handler: AiHandler = Arc::new(move |req: AiRequest| {
        ...
        if let Err(err) = std::thread::Builder::new()
            .name("manda-remote-ai".to_string())
            .spawn(move || run_chat(client, req))
        {
            ...
            log::warn!("manda-remote AI: failed to spawn handler thread: {err}");
    ...
    log::info!("manda-remote AI: handler registered");
}
...
fn run_chat(client: AiClient, req: AiRequest) {
    ...
    match result {
        ...
        Err(err) => {
            ...
            log::warn!("manda-remote AI: chat failed: {err:#}");
            let _ = tx.send(AiEvent::AiError {
                conversation_id,
...
const SYSTEM_PROMPT: &str = "You are MANDA, an assistant embedded inside a terminal emulator. \
