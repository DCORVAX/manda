...
/// Root of the soul directory: `$XDG_CONFIG_HOME/manda/soul/` (or
/// `~/.config/manda/soul/` when XDG is not set).
pub(crate) fn soul_dir() -> PathBuf {
    manda_config_dir().join("soul")
}
...
fn manda_config_dir() -> PathBuf {
    if let Some(xdg) = std::env::var_os("XDG_CONFIG_HOME") {
        PathBuf::from(xdg).join("manda")
    } else {
        let home = std::env::var("HOME").unwrap_or_else(|_| ".".into());
        PathBuf::from(home).join(".config").join("manda")
    }
}
...
fn migrate_v0_to_v1() {
    let legacy = manda_config_dir().join("ai_chat_memory.md");
    ...
    write_stub_if_absent(
        &soul_path(),
        "# About Me\n\n\
         <!-- Tell MANDA who you are. This file ships into every system prompt.\n\
    ...
    write_stub_if_absent(
        &skill_path(),
        "# Operating Modes\n\n\
         <!-- What you typically work on and how MANDA should help. -->\n",
    );
}
