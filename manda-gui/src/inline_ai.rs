...
pub(crate) const EVENT_PREFIX: &str = "manda-ai-inline-request:";
...
fn manda_state_dir() -> PathBuf {
    config::HOME_DIR.join(".config").join("manda")
}
...
pub(crate) fn spawn_job(job_id: &str) -> Result<()> {
    let jobs_dir = manda_state_dir().join("ai_jobs");
...
mod tests {
    ...
    fn bridge_contract_stays_in_sync_with_bundled_lua() {
        let lua = include_str!("../../assets/macos/Manda.app/Contents/Resources/manda.lua");
        ...
        assert!(lua.contains("manda_state_dir .. \"/ai_debug.log\""));
        assert!(lua.contains("wezterm.time.now():format_utc(\"%s%f\")"));
        assert!(lua.contains("cleanup_ai_fix_job_files(job)"));
        assert!(lua.contains("manda-toast-ai-clear-progress"));
        assert!(lua.contains("trusted_ai_last_command_by_pane"));
        assert!(!lua.contains("vars.manda_last_cmd"));
        assert!(!lua.contains("/tmp/manda_ai_debug.log"));
