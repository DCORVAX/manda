...
mod test {
    ...
    fn context_process_title_keeps_cwd_primary_without_spaces() {
        assert_eq!(
            context_process_title(Some("manda"), Some("codex")).as_deref(),
            Some("manda\u{00b7}codex")
        );
        assert_eq!(
            context_process_title(Some("manda"), None).as_deref(),
            Some("manda")
        );
        assert_eq!(
    ...
    fn multi_pane_separator_is_distinct_from_context_process_separator() {
        assert_eq!(
            ["www/manda\u{00b7}codex", "www/manda"].join(MULTI_PANE_TITLE_SEPARATOR),
            "www/manda\u{00b7}codex\u{2219}www/manda"
        );
