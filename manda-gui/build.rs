...
"#,
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let build_target_dir = if let Ok(target_dir) = std::env::var("CARGO_TARGET_DIR") {
            ...
            // OUT_DIR is like: target/release-opt/build/manda-gui-xxx/out
            // We need to get to: target/release-opt
            let out_dir = std::env::var("OUT_DIR").expect("OUT_DIR not set");
            let out_path = std::path::PathBuf::from(&out_dir);

            // Navigate up: out -> build -> manda-gui-xxx -> release-opt
        ...
        println!("cargo:rerun-if-changed=assets/macos/Manda.app/Contents/Info.plist");
