...
bundled `manda.lua` 设了 `front_end = 'WebGpu'`（Metal via wgpu）。`window/src/os/macos` 里的睡眠/唤醒防护（`SYSTEM_SLEEPING` flush 闸门、`BackendImpl::update`、display-change present defer）全部只作用于 CGL 路径，对默认用户是 no-op。给 sleep/wake/display-reconfig 加修复时必须同时考虑 WebGpu 路径（#458）。
...
- 不要调 `setWindowsMenu:` 让 AppKit 接管 Windows menu。MANDA 有自己的 tab/window 切换器。
- 不要调 `setHelpMenu:` 让 AppKit 注入 Help Search。MANDA 没有 help book。
...
新增 KeyAssignment 前确认走 MANDA 自己的按键分发路径，不要走 AppKit menu 的 `performKeyEquivalent`。后者会经过被 PAC-fault 的注入项。
...
- keyUp 不走 `performKeyEquivalent:`，所以 keyUp 照常进 MANDA。
...
兜底：任何"模仿系统快捷键"的 menu 项宁可 `keys: vec![]` 不设快捷键，让用户从菜单点。`manda-gui/src/commands.rs` 里的 keyEquivalent 装配逻辑必须统一从按键候选本身取 modifiers，不要为个别 menu 项走强制 modifier 的特例化路径（历史上的特例化路径已删，不要加回）。
...
`config.debug_key_events = true` 重启 MANDA，复现按键，看 `~/.local/share/manda/manda-gui-log-<pid>.txt`：
