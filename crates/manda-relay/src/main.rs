...
/// Desktop MANDA connects here and waits up to 10 minutes for a mobile client.
async fn host_ws(
...
async fn main() {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "manda_relay=info".into()),
        )
        .init();
    ...
    info!("manda-relay listening on {}", addr);

    let listener = match tokio::net::TcpListener::bind(addr).await {
        Ok(l) => l,
        Err(e) => {
            eprintln!("manda-relay: failed to bind {}: {}", addr, e);
            std::process::exit(1);
        }
    };
    if let Err(e) = axum::serve(listener, app).await {
        eprintln!("manda-relay: server error: {}", e);
        std::process::exit(1);
    }
}
