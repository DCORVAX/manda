...
fn builtin_manda_theme(config: &ConfigHandle) -> Option<CachedTheme> {
    ...
    match config.color_scheme.as_deref() {
        Some("MANDA Dark") | Some("MANDA Theme") => {
            // The color_scheme field might hold "MANDA Dark" either because:
            //   (a) the user explicitly chose MANDA Dark, OR
        ...
        Some("MANDA Light") => return Some(cached_theme(light)),
        _ => {}
    }
...
fn parse_color_scheme_selection_line(line: &str) -> Option<ColorSchemeSelection> {
    ...
    if rhs_starts_with_quoted_literal(rhs, "MANDA Light") {
        return Some(ColorSchemeSelection::Light);
    }
    if rhs_starts_with_quoted_literal(rhs, "MANDA Dark")
        || rhs_starts_with_quoted_literal(rhs, "MANDA Theme")
    {
        return Some(ColorSchemeSelection::Dark);
    }
    if rhs.contains("get_appearance") || rhs.contains("resolve_manda_color_scheme") {
...
fn theme_from_config(config: &ConfigHandle) -> CachedTheme {
    if let Some(theme) = builtin_manda_theme(config) {
        return theme;
    }
...
mod tests {
    ...
    fn detects_light_dark_and_auto_assignments() {
        assert_eq!(
            parse_color_scheme_selection_line("config.color_scheme = 'MANDA Light'"),
            Some(ColorSchemeSelection::Light)
        );
        assert_eq!(
            parse_color_scheme_selection_line("config.color_scheme = \"MANDA Dark\""),
            Some(ColorSchemeSelection::Dark)
        );
        assert_eq!(
            parse_color_scheme_selection_line(
                "config.color_scheme = (wezterm.gui and wezterm.gui.get_appearance() or 'Dark'):find('Dark') and 'MANDA Dark' or 'MANDA Light'"
            ),
            Some(ColorSchemeSelection::Auto)
        );
        // The bundled manda.lua uses resolve_manda_color_scheme which
