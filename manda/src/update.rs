...
mod imp {
    ...
    pub fn run(_assume_yes: bool) -> anyhow::Result<()> {
        bail!("`manda update` is currently supported on macOS only")
    }
}
...
mod imp {
    ...
    pub fn run(assume_yes: bool) -> anyhow::Result<()> {
        ...
        let new_app_path = find_manda_app(&extracted_dir).ok_or_else(|| {
            anyhow!(
                "update package `{}` does not contain `Manda.app`",
                UPDATE_ZIP_NAME
            )
        })?;
        ...
        let target_app = resolve_target_app_path().context("resolve installed Manda.app path")?;
        ...
        println!("MANDA will quit and relaunch automatically when replacement is complete.");
        Ok(())
    }

    fn resolve_update_provider() -> anyhow::Result<UpdateProvider> {
        if let Some(provider) = std::env::var_os("MANDA_UPDATE_PROVIDER") {
            let provider = provider.to_string_lossy().to_ascii_lowercase();
            return match provider.as_str() {
                "brew" => {
                    let brew_info = resolve_brew_info()?.ok_or_else(|| {
                        anyhow!(
                            "MANDA_UPDATE_PROVIDER=brew but brew-managed MANDA installation was not found"
                        )
                    })?;
                    Ok(UpdateProvider::Brew(brew_info))
                }
                "direct" => Ok(UpdateProvider::Direct),
                other => bail!("invalid MANDA_UPDATE_PROVIDER `{}`", other),
            };
        }
        ...
        if let Some(target) = std::env::var_os("MANDA_UPDATE_TARGET_APP") {
        ...
        if should_probe_brew {
            ...
            if find_brew_binary().is_none() {
                bail!(
                    "MANDA appears to be Homebrew-managed but `brew` was not found in PATH or standard locations"
                );
            }
        }
    ...
    fn resolve_brew_info() -> anyhow::Result<Option<BrewInfo>> {
        ...
        // Old cask name "manda" conflicts with another software in homebrew/cask.
        // Warn and fall back to direct update so existing users are not blocked.
        if is_brew_cask_installed(&brew_bin, "manda")? {
            println!(
                "WARNING: Detected old Homebrew cask 'manda' which conflicts with another software."
            );
            println!("Proceeding with direct update from GitHub for this run.");
            println!("Please migrate when convenient:");
            println!("  brew uninstall --cask manda");
    ...
    fn run_brew_upgrade(info: &BrewInfo, assume_yes: bool) -> anyhow::Result<()> {
        ...
        // app bundle and relaunches MANDA, which drops unsaved terminal work.
        ...
        if primary.success() {
            println!("Upgrade completed. Relaunching MANDA...");
            if !relaunch_after_upgrade() {
                println!("Upgrade completed. Please launch MANDA manually.");
            }
            return Ok(());
        }
    ...
    fn resolve_latest_tag_from_redirect(
        ...
        cmd.arg("--fail")
            ...
            .arg("10")
            .arg("--user-agent")
            .arg(format!("manda/{}", current_version))
            .arg("--write-out")
            .arg("%{url_effective}")
            .arg("--output")
            .arg("/dev/null")
    ...
    fn find_manda_app(extracted_dir: &Path) -> Option<PathBuf> {
        let direct = extracted_dir.join("Manda.app");
        if direct.exists() {
            return Some(direct);
        }
        ...
        for entry in entries.flatten() {
            ...
            if path
                .file_name()
                .and_then(|n| n.to_str())
                .map(|n| n.eq_ignore_ascii_case("Manda.app"))
                .unwrap_or(false)
            {
                return Some(path);
            }
    ...
    fn resolve_target_app_path() -> anyhow::Result<PathBuf> {
        if let Some(path) = std::env::var_os("MANDA_UPDATE_TARGET_APP") {
            let app = PathBuf::from(path);
            if app.ends_with("Manda.app") {
                return Ok(app);
            }
            bail!("MANDA_UPDATE_TARGET_APP must point to Manda.app");
        }
        ...
        let default_app = PathBuf::from("/Applications/Manda.app");
        if default_app.exists() {
            return Ok(default_app);
        }

        bail!("cannot locate installed Manda.app; run this from installed MANDA")
    }
    ...
    fn ensure_can_write_target(target_app: &Path) -> anyhow::Result<()> {
        ...
        let test_file = parent.join(format!(".manda-update-write-test-{}", now_unix_seconds()));
    ...
    /// Validates that the path points to a valid Manda.app bundle.
    /// Uses Path component comparison (not string suffix) to handle
    /// trailing separators correctly (e.g., /Applications/Manda.app/).
    fn validate_app_bundle_path(path: &Path) -> anyhow::Result<()> {
        // Use ends_with("Manda.app") which compares path components, not strings.
        // This correctly handles paths like /Applications/Manda.app/ (with trailing slash).
        if !path.ends_with("Manda.app") {
            bail!(
                "invalid app bundle path: must end with Manda.app: {}",
                path.display()
            );
        }
        Ok(())
    ...
    fn confirm_apply_update(update_label: &str, assume_yes: bool) -> anyhow::Result<bool> {
        ...
        if std::env::var_os("MANDA_UPDATE_AUTO_CONFIRM").is_some() {
            println!("Auto-confirming update (MANDA_UPDATE_AUTO_CONFIRM is set).");
            return Ok(true);
        }
        ...
        if !io::stdin().is_terminal() || !io::stdout().is_terminal() {
            ...
            bail!(
                "Refusing to apply a binary-replacing update in a non-interactive session.\n\
                 Re-run `manda update --yes` (or set MANDA_UPDATE_AUTO_CONFIRM=1) to proceed."
            );
        }
        ...
        println!("MANDA will quit and relaunch; running tasks will stop.");
    ...
    mod tests {
        ...
        fn release_notes_render_as_plain_text() {
            ...
            assert_eq!(strip_html_tags("<h1>MANDA V0.12.3</h1>"), "MANDA V0.12.3");
            assert_eq!(
        ...
        fn brew_and_direct_paths_confirm_before_replace() {
            ...
            assert!(
                src.contains("MANDA will quit and relaunch; running tasks will stop."),
