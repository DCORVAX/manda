...
impl Tool {
    fn label(&self) -> &'static str {
        match self {
            Tool::MandaAssistant => "MANDA Assistant",
    ...
    fn config_path(&self) -> PathBuf {
        ...
        match self {
            Tool::MandaAssistant => assistant_config::assistant_toml_path().unwrap_or_else(|_| {
                config::HOME_DIR
                    .join(".config")
                    .join("manda")
                    .join("assistant.toml")
...
impl ToolState {
    ...
    fn load_with_usage(tool: Tool, eager_remote_usage: bool) -> Self {
        let path = if tool == Tool::MandaAssistant {
        ...
        if tool != Tool::MandaAssistant && !path.exists() && !extra_exists {
            return ToolState {
                tool,
                installed: false,
                fields: Vec::new(),
        ...
        let (fields, usage_summary) = match tool {
            Tool::MandaAssistant => (extract_manda_assistant_fields(&raw), None),
            Tool::ClaudeCode => {
...
/// Configuration for the MANDA built-in AI assistant.
///
/// This struct holds the configuration for MANDA's AI-powered command analysis
...
impl MandaAssistantConfig {
    ...
    fn with_chat_model_passthrough(mut self, src: &MandaAssistantConfig) -> Self {
    ...
    fn with_chat_model_choices_passthrough(mut self, src: &MandaAssistantConfig) -> Self {
        self.chat_model_choices = src.chat_model_choices.clone();
        self
    }

    /// Replace the curated deep-model choices outright, used when switching to a
    /// MANDA provider preset so the GUI overlay's model picker matches the preset.
    fn with_chat_model_choices(mut self, choices: Vec<String>) -> Self {
        self.chat_model_choices = choices;
        self
    }
...
fn parse_manda_assistant_config(raw: &str) -> MandaAssistantConfig {
    ...
    let mut cfg = MandaAssistantConfig::new(enabled, api_key, simple_model, base_url)
...
fn get_manda_assistant_api_key() -> Option<String> {
    ...
    let raw = std::fs::read_to_string(&path)
        ...
        .ok()?;
    let cfg = parse_manda_assistant_config(&raw);
...
// Delegated to manda-ai-utils crate to avoid cross-binary drift.
...
fn fetch_manda_assistant_models(api_key: &str, base_url: &str) -> Vec<String> {
    ...
    let mut models: Vec<String> = v
        ...
        .map(|arr| {
            arr.iter()
                ...
                .filter(|id| manda_ai_utils::is_chat_model_id(id))
                .collect()
        })
        .unwrap_or_default();
...
fn assistant_model_options_for_config(cfg: &MandaAssistantConfig) -> Vec<String> {
    let cache_path = assistant_models_cache_path();
...
fn assistant_model_options_for_config_remote(cfg: &MandaAssistantConfig) -> Vec<String> {
    let mut models = fetch_manda_assistant_models(cfg.api_key(), cfg.base_url());
...
fn extract_manda_assistant_fields_with_model_options(
    raw: &str,
    ...
    let mut fields = vec![
        ...
        // MANDA's five built-in OpenAI-compatible provider presets. Selecting one
        // fills Base URL / models and points the key at its env var; "Custom" keeps
        // the current values for any other OpenAI-compatible endpoint.
        FieldEntry {
            key: "Provider".into(),
            value: manda_ai_utils::providers::provider_label_for_base_url(cfg.base_url())
                .to_string(),
            options: manda_ai_utils::providers::provider_options(),
            editable: true,
        },
        FieldEntry {
            key: "Auth Type".into(),
...
fn extract_manda_assistant_fields(raw: &str) -> Vec<FieldEntry> {
    let cfg = parse_manda_assistant_config(raw);
    ...
    extract_manda_assistant_fields_with_model_options(raw, model_options)
}
...
fn render_manda_assistant_config(cfg: &MandaAssistantConfig) -> String {
    let mut out = String::new();
    out.push_str("# MANDA Assistant configuration\n");
    out.push_str(
...
#[cfg(test)]
fn write_manda_assistant_config(path: &Path, cfg: &MandaAssistantConfig) -> anyhow::Result<()> {
    let out = render_manda_assistant_config(cfg);
...
fn write_manda_assistant_config_preserving(
    path: &Path,
    cfg: &MandaAssistantConfig,
    original_raw: &str,
) -> anyhow::Result<()> {
    ...
    let canonical = render_manda_assistant_config(cfg);
...
fn save_manda_assistant_field(field_key: &str, new_val: &str) -> anyhow::Result<()> {
    let path = assistant_config::ensure_assistant_toml_exists()?;
    save_manda_assistant_field_to_path(&path, field_key, new_val)
}

pub(crate) fn save_manda_assistant_field_to_path(
    path: &Path,
    field_key: &str,
    new_val: &str,
) -> anyhow::Result<()> {
    ...
    let mut updated = match field_key {
        "Enabled" => {
            ...
            MandaAssistantConfig::new(enabled, cfg.api_key(), cfg.model(), cfg.base_url())
        ...
        "Provider" => {
            // Selecting a MANDA preset applies its base URL, default simple/deep
            // models, curated deep-model choices, and clears the stored api_key
            // so the key resolves from the provider's env var (env-first).
            // "Custom" keeps every existing value untouched.
            match manda_ai_utils::providers::find_provider_by_label(new_val.trim()) {
                Some(preset) => {
                    MandaAssistantConfig::new(cfg.is_enabled(), "", preset.model, preset.base_url)
                        .with_custom_headers(cfg.custom_headers().to_vec())
                        .with_web_search(cfg.web_search_provider(), cfg.web_search_api_key())
                        .with_chat_model(preset.chat_model)
                        .with_chat_model_choices(
                            preset
                                .chat_model_choices
                                .iter()
                                .map(|s| (*s).to_string())
        ...
        "Simple Model" | "Model" => {
            ...
            MandaAssistantConfig::new(cfg.is_enabled(), cfg.api_key(), model, cfg.base_url())
                .with_custom_headers(cfg.custom_headers().to_vec())
                .with_web_search(cfg.web_search_provider(), cfg.web_search_api_key())
                .with_chat_model_passthrough(&cfg)
        }
        "Deep Model" | "Chat Model" => {
            ...
            MandaAssistantConfig::new(cfg.is_enabled(), cfg.api_key(), cfg.model(), cfg.base_url())
                .with_custom_headers(cfg.custom_headers().to_vec())
        ...
        "Base URL" => {
            ...
            MandaAssistantConfig::new(cfg.is_enabled(), cfg.api_key(), cfg.model(), base_url)
                .with_custom_headers(cfg.custom_headers().to_vec())
                .with_web_search(cfg.web_search_provider(), cfg.web_search_api_key())
                .with_chat_model_passthrough(&cfg)
        }
        ...
        "API Key" => MandaAssistantConfig::new(
            cfg.is_enabled(),
            new_val.trim(),
            cfg.model(),
            cfg.base_url(),
    ...
    write_manda_assistant_config_preserving(path, &updated, &raw)
}
...
/// Fetch models.dev data, cached to ~/.cache/manda/models.json.
/// No TTL - use `r` key in TUI to force refresh.
fn load_models_dev_json() -> Option<serde_json::Value> {
    let cache_dir = config::HOME_DIR.join(".cache").join("manda");
...
impl App {
    ...
    fn load_tools() -> Vec<ToolState> {
        ALL_TOOLS
            ...
            .filter(|t| t.tool == Tool::MandaAssistant || t.installed)
            .collect()
    }
    ...
    fn tool_row_count(&self, tool_index: usize) -> usize {
        ...
        if tool.tool == Tool::MandaAssistant {
    ...
    fn tool_is_collapsed(&self, tool_index: usize) -> bool {
        self.tools
            .get(tool_index)
            .is_some_and(|tool| tool.tool == Tool::MandaAssistant && self.assistant_collapsed)
    }
    ...
    fn toggle_manda_assistant_collapsed(&mut self) {
        if !self
            .tools
            .iter()
            .any(|tool| tool.tool == Tool::MandaAssistant && !tool.fields.is_empty())
        {
            return;
        }
        ...
        if self
            .current_tool()
            .is_some_and(|tool| tool.tool == Tool::MandaAssistant)
        {
            self.field_index = 0;
        }
        self.set_status(if self.assistant_collapsed {
            "MANDA Assistant hidden"
        } else {
            "MANDA Assistant expanded"
        });
    }
    ...
    fn rendered_tool_row_count(&self) -> usize {
        self.tools
            .iter()
            .map(|tool| {
                1 + if tool.tool == Tool::MandaAssistant && self.assistant_collapsed {
                    1
                } else {
                    tool.fields.len() + 1
                }
    ...
    fn start_edit(&mut self) {
        if self
            .current_tool()
            .is_some_and(|tool| tool.tool == Tool::MandaAssistant && self.field_index == 0)
        {
            self.toggle_manda_assistant_collapsed();
            return;
        }
        ...
        let edit_buf = if field.value == "-" {
            // Empty placeholder
            String::new()
        } else if tool.tool == Tool::MandaAssistant && field.key == "API Key" {
            get_manda_assistant_api_key().unwrap_or_else(String::new)
        } else {
            field.value.clone()
        };
    ...
    fn confirm_edit(&mut self) {
        ...
        self.field_index = if tool_kind == Tool::MandaAssistant {
            field_idx + 1
        } else {
            field_idx
        };
    ...
    fn reload_current_tool_focusing(&mut self, focus_key: Option<&str>) {
        ...
        if tool_type == Tool::MandaAssistant {
        ...
        self.field_index = match new_field_idx {
            ...
            None if tool_type == Tool::MandaAssistant => 0,
            None => 0,
        };
    ...
    /// Open a shell command in a new MANDA tab (preferred) or fall back to Terminal.app.
    fn open_in_terminal(&mut self, cmd: &str) {
        // Prefer a new tab in the current MANDA window.
        // manda cli spawn reads WEZTERM_PANE from the environment to target the right window.
        // Append `exec $SHELL` so the pane stays alive after the command finishes.
        let shell = std::env::var("SHELL").unwrap_or_else(|_| "/bin/zsh".to_string());
        let shell_cmd = format!("{}; exec \"{}\"", cmd, shell);
        let manda_status = std::process::Command::new("manda")
            .args(["cli", "spawn", "--", &shell, "-c", &shell_cmd])
            .stdin(std::process::Stdio::null())
            .stdout(std::process::Stdio::null())
            .stderr(std::process::Stdio::null())
            .status();
        if manda_status.as_ref().is_ok_and(|status| status.success()) {
            self.set_status("Opening in new MANDA tab...");
            return;
        }
...
fn save_field(tool: Tool, field_key: &str, new_val: &str) -> anyhow::Result<()> {
    if tool == Tool::MandaAssistant {
        return save_manda_assistant_field(field_key, new_val);
    }
    ...
    match tool {
        Tool::MandaAssistant => unreachable!("MANDA Assistant is handled before JSON parsing"),
    ...
    if is_jsonc_path(&path) {
        log::info!(
            "Note: {} comments will be removed when MANDA rewrites this file.",
            path.display()
        );
    }
...
mod tests {
    ...
    #[test]
    fn manda_assistant_fields_include_enabled_toggle() {
        let fields = extract_manda_assistant_fields("enabled = true\n");
        assert!(fields.iter().any(|field| field.key == "Enabled"));
        let fields = extract_manda_assistant_fields("enabled = false\n");
        assert!(fields.iter().any(|field| field.key == "Enabled"));
    }

    #[test]
    fn manda_assistant_model_fields_accept_custom_select_values() {
    ...
    #[test]
    fn summarize_manda_assistant_requires_api_key() {
        let fields = extract_manda_assistant_fields("enabled = false\n");
        assert_eq!(
            summarize_tool_fields(Tool::MandaAssistant, true, &fields, None),
            Some(format!(
                "Setup required · {}",
                assistant_config::DEFAULT_MODEL
            ))
    ...
    #[test]
    fn summarize_manda_assistant_prefers_status_and_model() {
        let fields = extract_manda_assistant_fields(
            "enabled = true\napi_key = \"sk-test\"\nmodel = \"gpt-5-mini\"\n",
        );
        assert_eq!(
            summarize_tool_fields(Tool::MandaAssistant, true, &fields, None),
...
"#;
        ...
        assert_eq!(parse_claude_keychain_account(raw), Some("alice".into()));
    }
...
"#,
    ...
    #[test]
    fn manda_assistant_fields_show_simple_and_deep_models() {
        let fields = extract_manda_assistant_fields(
            "enabled = true\napi_key = \"sk-test\"\nmodel = \"gpt-5.4-mini\"\nbase_url = \"https://api.openai.com/v1\"\n",
        );
        // Provider row is always present for non-codex configs.
        assert!(fields.iter().any(|f| f.key == "Provider"));
    ...
    #[test]
    fn manda_assistant_legacy_fast_model_collapses_into_simple_model() {
        let raw = "enabled = true\napi_key = \"sk-test\"\nmodel = \"gpt-5.4\"\nfast_model = \"gpt-5.4-mini\"\nchat_model = \"gpt-5.5\"\nbase_url = \"https://api.openai.com/v1\"\n";
        let fields = extract_manda_assistant_fields(raw);
        ...
        write_manda_assistant_config(&path, &cfg).expect("write config");
    ...
    #[test]
    fn manda_assistant_legacy_fast_model_preserves_old_model_as_deep_model() {
        let raw = "enabled = true\napi_key = \"sk-test\"\nmodel = \"gpt-5.4\"\nfast_model = \"gpt-5.4-mini\"\nbase_url = \"https://api.openai.com/v1\"\n";
        let fields = extract_manda_assistant_fields(raw);
    ...
    #[test]
    fn manda_assistant_save_base_url_and_model_round_trip() {
        let dir = tempdir().expect("tempdir");
        let path = dir.path().join("assistant.toml");
        std::fs::write(
            &path,
        ...
        let cfg = parse_manda_assistant_config(&raw);
        let mut updated = MandaAssistantConfig::new(
            cfg.is_enabled(),
            cfg.api_key(),
            "my-model",
        ...
        write_manda_assistant_config(&path, &updated).expect("write config");
    ...
    #[test]
    fn manda_assistant_save_auth_type_codex_round_trip() {
        let dir = tempdir().expect("tempdir");
        let path = dir.path().join("assistant.toml");
        std::fs::write(
            &path,
        ...
        save_manda_assistant_field_to_path(&path, "Auth Type", "codex").expect("save auth type");
        let saved = std::fs::read_to_string(&path).expect("read saved");
        assert!(
        ...
        let fields = extract_manda_assistant_fields(&saved);
        assert!(fields
            .iter()
        ...
        // Auth Type sits right after Provider (which sits after Enabled).
        assert_eq!(fields.first().map(|f| f.key.as_str()), Some("Enabled"));
        assert_eq!(fields.get(1).map(|f| f.key.as_str()), Some("Provider"));
        assert_eq!(fields.get(2).map(|f| f.key.as_str()), Some("Auth Type"));

        // Switching back to api_key drops the auth_type line and restores the field.
        save_manda_assistant_field_to_path(&path, "Auth Type", "api_key").expect("save api_key");
        let saved2 = std::fs::read_to_string(&path).expect("read saved2");
        assert!(!saved2.contains("auth_type ="));
        let fields2 = extract_manda_assistant_fields(&saved2);
        assert!(fields2.iter().any(|f| f.key == "API Key"));
    }

    #[test]
    fn manda_assistant_responses_and_native_search_round_trip() {
        let dir = tempdir().expect("tempdir");
        let path = dir.path().join("assistant.toml");
        std::fs::write(
            &path,
        ...
        save_manda_assistant_field_to_path(&path, "API Mode", "responses")
        ...
        save_manda_assistant_field_to_path(&path, "Native Web Search", "On")
            .expect("enable native search");
        save_manda_assistant_field_to_path(&path, "Simple Model", "gpt-5.4")
        ...
        save_manda_assistant_field_to_path(&path, "API Mode", "chat_completions")
    ...
    #[test]
    fn manda_assistant_save_preserves_chat_model_keys() {
        ...
        let cfg2 = parse_manda_assistant_config(&saved);
        assert_eq!(cfg2.chat_model(), "gpt-5.4");
        assert_eq!(cfg2.chat_model_choices(), &["gpt-5.4", "claude-sonnet-4-6"]);
    }

    #[test]
    fn manda_assistant_tui_save_preserves_runtime_and_unknown_keys() {
        let dir = tempdir().expect("tempdir");
        let path = dir.path().join("assistant.toml");
        std::fs::write(
            &path,
        ...
        save_manda_assistant_field_to_path(&path, "Simple Model", "gpt-5.4").expect("save model");
        let saved = std::fs::read_to_string(&path).expect("read saved config");
    ...
    #[test]
    fn manda_assistant_tui_save_preserves_comments() {
        let dir = tempdir().expect("tempdir");
        let path = dir.path().join("assistant.toml");
        std::fs::write(
            &path,
            concat!(
                "# MANDA assistant configuration\n",
                "enabled = true\n",
                "model = \"gpt-5.4-mini\"\n",
        ...
        assert!(
            saved.contains("# MANDA assistant configuration"),
            "{}",
            saved
        );
        assert!(
    ...
    #[test]
    fn manda_assistant_auto_fix_ignored_exit_codes_parse_and_normalize() {
        let cfg = parse_manda_assistant_config(
        ...
        let cfg = parse_manda_assistant_config("enabled = true\nmodel = \"gpt-5.4-mini\"\n");
        assert!(cfg.auto_fix_ignored_exit_codes().is_empty());
    }

    #[test]
    fn manda_assistant_save_preserves_auto_fix_ignored_exit_codes() {
        let dir = tempdir().expect("tempdir");
        let path = dir.path().join("assistant.toml");
        std::fs::write(
            &path,
        ...
        save_manda_assistant_field_to_path(&path, "Base URL", "https://proxy.example.com/v1")
    ...
    #[test]
    fn manda_assistant_deep_model_field_is_editable() {
        let dir = tempdir().expect("tempdir");
        let path = dir.path().join("assistant.toml");
        std::fs::write(
            &path,
        ...
        save_manda_assistant_field_to_path(&path, "Deep Model", "claude-sonnet-4-6")
    ...
    #[test]
    fn manda_assistant_deep_model_clears_when_dash_selected() {
        ...
        save_manda_assistant_field_to_path(&path, "Deep Model", "-")
    ...
    #[test]
    fn manda_assistant_save_omits_chat_model_when_unset() {
    ...
    #[test]
    fn manda_assistant_provider_round_trip_preserves_headers() {
