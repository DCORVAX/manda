...
fn render_tools(frame: &mut ratatui::Frame, area: Rect, app: &App) {
    ...
    for (ti, tool) in app.tools.iter().enumerate() {
        ...
        let header_selected =
            is_current_tool && tool.tool == Tool::MandaAssistant && app.field_index == 0;

        let tool_style =
            if header_selected || (is_current_tool && tool.tool != Tool::MandaAssistant) {
                Style::default().fg(primary()).add_modifier(Modifier::BOLD)
        ...
        for (fi, field) in tool.fields.iter().enumerate() {
            let display_index = if tool.tool == Tool::MandaAssistant {
                fi + 1
            } else {
                fi
            };
