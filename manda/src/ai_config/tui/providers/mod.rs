//! Per-provider usage scraping and quota formatting for the `manda ai` TUI.
//!
...
use super::{
    ...
    extract_antigravity_fields, extract_manda_assistant_fields_with_model_options,
    kimi_credentials_path, parse_manda_assistant_config, read_codex_model_options,
...
pub(super) fn load_antigravity_usage_snapshot() -> Option<AntigravityUsageSnapshot> {
    ...
    // Antigravity model selection can change out of band while MANDA is open,
    ...
    match tool {
        Tool::MandaAssistant => {
            ...
            UsageSummaryUpdate {
                tool,
                summary: None,
                fields: Some(extract_manda_assistant_fields_with_model_options(
                    &raw,
                    model_options,
                )),
            }
