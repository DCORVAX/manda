...
mod imp {
    ...
    pub fn run(_yes: bool, _shell: Option<crate::shell::ManagedShell>) -> anyhow::Result<()> {
        bail!("`manda reset` is currently supported on macOS only")
    }
}
...
mod imp {
    ...
    const MANDA_SOURCE_PATTERN: &str = "manda/zsh/manda.zsh";
    const MANDA_PATH_MARKER: &str = "# MANDA PATH Integration";
    const MANDA_TMUX_SOURCE_PATTERN: &str = "manda/tmux/manda.tmux.conf";
    const MANDA_LEGACY_INLINE_MARKER: &str = "# MANDA Shell Integration";
    const MANDA_LEGACY_INLINE_VAR: &str = "MANDA_ZSH_DIR";
    const MANDA_LEGACY_SYNTAX_HINT: &str = "zsh-syntax-highlighting/zsh-syntax-highlighting.zsh";
    // SYNC: this list must stay in sync with the heredoc in
    // assets/shell-integration/setup_zsh.sh legacy_inline_block_has_only_manda_managed_lines().
    // When adding or removing lines, update both places.
    const MANDA_LEGACY_INLINE_KNOWN_LINES: &[&str] = &[
        "# MANDA Zsh Integration - DO NOT EDIT MANUALLY",
        "# This file is managed by Manda.app. Any changes may be overwritten.",
        r#"export MANDA_ZSH_DIR="$HOME/.config/manda/zsh""#,
        "# Add bundled binaries to PATH",
        r#"export PATH="$MANDA_ZSH_DIR/bin:$PATH""#,
        "# Initialize Starship (Cross-shell prompt)",
        "# Check file existence to avoid \"no such file\" errors in some zsh configurations",
        r#"if [[ -x "$MANDA_ZSH_DIR/bin/starship" ]]; then"#,
        r#"    eval "$("$MANDA_ZSH_DIR/bin/starship" init zsh)""#,
        ...
        r#"if [[ -d "$MANDA_ZSH_DIR/plugins/zsh-completions/src" ]]; then"#,
        r#"    fpath=("$MANDA_ZSH_DIR/plugins/zsh-completions/src" $fpath)"#,
        ...
        r#"if [[ -f "$MANDA_ZSH_DIR/plugins/zsh-z/zsh-z.plugin.zsh" ]]; then"#,
        ...
        r#"    : "${ZSHZ_CASE:=smart}""#,
        "    export ZSHZ_CASE",
        r#"    source "$MANDA_ZSH_DIR/plugins/zsh-z/zsh-z.plugin.zsh""#,
        "# MANDA defers autosuggestions to the external provider detected during manda init.",
        r#"typeset -g _manda_autosuggest_cli_provider="""#,
        r#"typeset -g _manda_autosuggest_cli_provider="kiro-cli""#,
        r#"typeset -g _manda_autosuggest_cli_provider="q""#,
        ...
        r#"if [[ -n "${_manda_autosuggest_cli_provider:-}" ]]; then"#,
        ...
        r#"if ! (( ${+functions[_zsh_autosuggest_start]} )) && [[ "${_manda_external_autosuggest_provider:-0}" != "1" ]] && [[ -f "$MANDA_ZSH_DIR/plugins/zsh-autosuggestions/zsh-autosuggestions.zsh" ]]; then"#,
        ...
        r#"if [[ -f "$MANDA_ZSH_DIR/plugins/zsh-autosuggestions/zsh-autosuggestions.zsh" ]]; then"#,
        r#"    source "$MANDA_ZSH_DIR/plugins/zsh-autosuggestions/zsh-autosuggestions.zsh""#,
        ...
        r#"        if [[ "${MANDA_TAB_ACCEPT_SUGGEST_FIRST:-0}" == "1" ]] && [[ -n "$POSTDISPLAY" ]]; then"#,
        ...
        r#"    bindkey -M emacs '^I' manda_tab_accept_or_complete"#,
        r#"    bindkey -M main '^I' manda_tab_accept_or_complete"#,
        r#"    bindkey -M viins '^I' manda_tab_accept_or_complete"#,
        ...
        r#"source "$MANDA_ZSH_DIR/plugins/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh""#,
        r#"if [[ -f "$MANDA_ZSH_DIR/plugins/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh" ]]; then"#,
        ...
        "# This plugin must be loaded LAST, and we delay it for faster shell startup.",
        r#"if ! (( ${+functions[_zsh_highlight]} )) && [[ -f "$MANDA_ZSH_DIR/plugins/fast-syntax-highlighting/fast-syntax-highlighting.plugin.zsh" ]]; then"#,
        "    fast_syntax_highlighting_defer() {",
        r#"        source "$MANDA_ZSH_DIR/plugins/fast-syntax-highlighting/fast-syntax-highlighting.plugin.zsh""#,
        r#"        precmd_functions=("${precmd_functions[@]:#fast_syntax_highlighting_defer}")"#,
        "    }",
        "    precmd_functions+=(fast_syntax_highlighting_defer)",
    ];

    const MANDA_GIT_DEFAULTS: &[(&str, &str)] = &[
        ("core.pager", "delta"),
    ...
    pub fn run(yes: bool, shell: Option<ManagedShell>) -> anyhow::Result<()> {
        ...
        remove_manda_shell_dir(&mut report)?;
        remove_fish_integration(&mut report)?;
        remove_tmux_integration(&mut report)?;
        remove_file_if_exists(
            home_manda_dir().join("tmux").join("manda.tmux.conf"),
        ...
        remove_file_if_exists(
            ...
            "removed persisted MANDA state",
            &mut report,
        )?;
        remove_file_if_exists(
            config_home().join(".manda_config_version"),
            "removed legacy MANDA config version marker",
            &mut report,
        )?;
        remove_file_if_exists(
            config_home().join(".manda_window_geometry"),
            "removed legacy MANDA window geometry marker",
            &mut report,
        )?;
        ...
        if home_manda_dir() != config_home() {
            for (name, label) in [
                (
                    "state.json",
                    "removed persisted MANDA state (default location)",
                ),
                (
                    ".manda_config_version",
                    "removed legacy MANDA config version marker (default location)",
                ),
                (
                    ".manda_window_geometry",
                    "removed legacy MANDA window geometry marker (default location)",
                ),
            ] {
                remove_file_if_exists(home_manda_dir().join(name), label, &mut report)?;
            }
        }
        remove_file_if_exists(
            home_manda_dir().join("lazygit_state.json"),
            "removed Lazygit hint state",
            &mut report,
        )?;
        remove_dir_if_exists(
            config_home().join("backups"),
            "removed MANDA backup directory",
            &mut report,
        )?;
        remove_dir_if_exists(
            config_home().join("soul"),
            "removed MANDA soul directory",
            &mut report,
        )?;
        remove_file_if_exists(
        ...
        remove_empty_manda_config_dir(&mut report)?;
        ...
        let restore_cmd = format!("manda init --shell {}", selected_shell.name());
    ...
    fn confirm_reset(yes: bool) -> anyhow::Result<()> {
        ...
        println!(
            "This will remove MANDA shell and tmux integration and reset MANDA-managed git defaults."
        );
    ...
    fn config_home() -> PathBuf {
        config::CONFIG_DIRS
            .first()
            .cloned()
            .unwrap_or_else(|| home_dir().join(".config").join("manda"))
    }

    // Shell assets and the legacy Lazygit hint state are intentionally anchored
    // at `$HOME/.config/manda`. Keep this separate from `config_home()`, which
    // follows XDG_CONFIG_HOME for application state and manda.lua.
    fn home_manda_dir() -> PathBuf {
        home_manda_dir_for(&home_dir())
    }

    fn home_manda_dir_for(home: &Path) -> PathBuf {
        home.join(".config").join("manda")
    }
    ...
    fn is_active_manda_tmux_source_line(trimmed_line: &str) -> bool {
        if trimmed_line.starts_with('#') || !trimmed_line.contains(MANDA_TMUX_SOURCE_PATTERN) {
            return false;
        }
    ...
    fn remove_zsh_integration(report: &mut ResetReport) -> anyhow::Result<()> {
        ...
        let filtered: Vec<&str> = original
            .lines()
            .filter(|line| {
                !line.contains(MANDA_SOURCE_PATTERN) && !line.contains(MANDA_PATH_MARKER)
            })
            .collect();
        ...
        if !removed_managed_lines && !removed_legacy_block {
            report.skipped(format!(
                "no MANDA shell integration found in {}",
                zshrc.display()
            ));
            return Ok(());
        }
        ...
        if removed_managed_lines && removed_legacy_block {
            report.changed(format!(
                "removed MANDA-managed .zshrc lines and legacy inline block from {}",
                zshrc.display()
            ));
        } else if removed_managed_lines {
            report.changed(format!(
                "removed MANDA-managed .zshrc lines from {}",
                zshrc.display()
            ));
        } else {
            report.changed(format!(
                "removed legacy inline MANDA block from {}",
                zshrc.display()
            ));
        }
        Ok(())
    ...
    fn remove_manda_shell_dir(report: &mut ResetReport) -> anyhow::Result<()> {
        let manda_init = home_manda_dir().join("zsh").join("manda.zsh");
        if manda_init.exists() {
            std::fs::remove_file(&manda_init)
                .with_context(|| format!("remove {}", manda_init.display()))?;
            report.changed(format!("removed {}", manda_init.display()));
        } else {
            report.skipped(format!("{} not found", manda_init.display()));
        }
        Ok(())
    }
    ...
    fn remove_fish_integration(report: &mut ResetReport) -> anyhow::Result<()> {
        ...
        remove_file_if_exists(
            conf_d_file,
            "removed ~/.config/fish/conf.d/manda.fish",
            report,
        )?;

        // Remove managed fish init file
        let fish_init = home_manda_dir().join("fish").join("manda.fish");
        remove_file_if_exists(fish_init, "removed ~/.config/manda/fish/manda.fish", report)?;

        // Remove fish wrapper bin
        let fish_wrapper = home_manda_dir().join("fish").join("bin").join("manda");
        remove_file_if_exists(
            fish_wrapper,
            "removed ~/.config/manda/fish/bin/manda wrapper",
            report,
        )?;
    ...
    fn remove_tmux_integration(report: &mut ResetReport) -> anyhow::Result<()> {
        ...
        let filtered: Vec<&str> = original
            .lines()
            .filter(|line| !is_active_manda_tmux_source_line(line.trim_start()))
            .collect();

        if filtered.len() == original.lines().count() {
            report.skipped(format!(
                "no MANDA tmux integration found in {}",
                tmuxrc.display()
            ));
            return Ok(());
        }
        ...
        report.changed(format!(
            "removed MANDA-managed tmux source line from {}",
            tmuxrc.display()
        ));
        Ok(())
    }
    ...
    fn cleanup_git_delta_defaults(report: &mut ResetReport) -> anyhow::Result<()> {
        ...
        for (key, expected) in MANDA_GIT_DEFAULTS {
        ...
        if removed.is_empty() {
            report.skipped("no MANDA-managed git defaults to remove");
        } else {
    ...
    fn cleanup_theme_block(report: &mut ResetReport) -> anyhow::Result<()> {
        let config_path = config_home().join("manda.lua");
        ...
        let (after_managed, changed_managed) =
            strip_theme_block(&original, "-- ===== MANDA Theme Defaults (managed) =====");
        let (after_legacy, changed_legacy) =
            strip_theme_block(&after_managed, "-- ===== MANDA Theme =====");

        if !changed_managed && !changed_legacy {
            report.skipped("no managed MANDA theme block found in ~/.config/manda/manda.lua");
            return Ok(());
        }

        std::fs::write(&config_path, after_legacy)
            .with_context(|| format!("write {}", config_path.display()))?;
        report.changed("removed managed MANDA theme block from ~/.config/manda/manda.lua");
        Ok(())
    }
    ...
    /// Strips the old inline MANDA shell integration block that was written directly
    /// into .zshrc in early MANDA versions (before the source-based manda.zsh approach).
    /// The block starts with `# MANDA Shell Integration`, contains `MANDA_ZSH_DIR`, and
    ...
    fn strip_legacy_inline_block(content: &str) -> (String, bool) {
        ...
        while i < lines.len() {
            ...
            if current == MANDA_LEGACY_INLINE_MARKER {
                let mut j = i + 1;
                let mut saw_manda_var = false;
                let mut saw_syntax_line = false;
                let mut end = None;

                while j < lines.len() {
                    let line = lines[j];
                    if line.contains(MANDA_LEGACY_INLINE_VAR) {
                        saw_manda_var = true;
                    }
                    if line.contains(MANDA_LEGACY_SYNTAX_HINT) {
    ...
    fn is_known_legacy_inline_line(line: &str) -> bool {
        ...
        trimmed.is_empty() || MANDA_LEGACY_INLINE_KNOWN_LINES.contains(&trimmed)
    }
    ...
    fn remove_empty_manda_config_dir(report: &mut ResetReport) -> anyhow::Result<()> {
        let dir = config_home();
        if !dir.exists() {
            return Ok(());
        }
    ...
    mod tests {
        use super::{
            home_manda_dir_for, is_active_manda_tmux_source_line, strip_legacy_inline_block,
            MANDA_TMUX_SOURCE_PATTERN,
        };
        use std::path::Path;

        #[test]
        fn home_anchored_manda_files_stay_under_home_dot_config() {
            assert_eq!(
                home_manda_dir_for(Path::new("/Users/tester")),
                Path::new("/Users/tester/.config/manda")
            );
        }

        #[test]
        fn active_tmux_source_line_is_detected() {
            let line = r#"source-file "$HOME/.config/manda/tmux/manda.tmux.conf" # MANDA tmux Integration"#;
            assert!(is_active_manda_tmux_source_line(line));
        }

        #[test]
        fn commented_tmux_source_line_is_ignored() {
            let line = r#"# source-file "$HOME/.config/manda/tmux/manda.tmux.conf""#;
            assert!(!is_active_manda_tmux_source_line(line));
        }

        #[test]
        fn plain_text_reference_is_ignored() {
            let line = format!("note: {}", MANDA_TMUX_SOURCE_PATTERN);
            assert!(!is_active_manda_tmux_source_line(&line));
        }
