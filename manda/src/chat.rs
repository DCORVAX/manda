//! `manda chat` subcommand: thin wrapper around the bundled `k` binary.
//!
//! Why this exists when `k` is already on PATH after `manda init`:
//!
//! - **Discoverability** — `manda --help` should show the full AI story
...
//!   `manda init` or is on a shell without managed integration.
//! - **No dependency bloat** — instead of pulling the GUI/chat engine into
//!   the `manda` binary (which would link in WGPU + Cairo + the entire
//!   GUI runtime), we `execvp` the `k` binary that already ships in
//!   `Manda.app/Contents/MacOS/m`. Same chat engine, same args, zero
...
/// Locate the `k` binary that ships next to this `manda` binary, falling back
/// to common install locations. Mirrors the discovery order used by
/// `init.rs` so behavior stays predictable across `manda init`, `manda chat`,
/// and the standalone `k` shell wrapper.
fn resolve_k_binary() -> anyhow::Result<PathBuf> {
    let mut candidates: Vec<PathBuf> = Vec::new();

    // 1. Same directory as the running `manda` binary (the bundled case).
    if let Ok(exe) = std::env::current_exe() {
        candidates.push(exe.with_file_name("m"));
        if let Ok(canonical) = std::fs::canonicalize(&exe) {
            let beside_canonical = canonical.with_file_name("m");
    ...
    #[cfg(target_os = "macos")]
    {
        candidates.push(PathBuf::from("/Applications/Manda.app/Contents/MacOS/m"));
        candidates.push(
            config::HOME_DIR
                .join("Applications")
                .join("Manda.app")
    ...
    Err(anyhow!(
        "could not locate the `k` chat binary. Run `manda init` after \
         installing MANDA, or invoke `k` directly if it is on PATH."
    ))
}
...
mod tests {
    ...
    fn resolve_returns_error_when_no_candidate_exists() {
        ...
        // result. CI machines without MANDA installed should hit the Err
