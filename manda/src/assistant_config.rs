//! MANDA Assistant configuration management.
//!
//! This module handles the configuration file for MANDA's built-in AI assistant,
//! including default values, file paths, and ensuring required configuration keys exist.
//!
//! The configuration is stored in `assistant.toml` in the user's MANDA config directory.
...
// in `manda-gui/src/ai_client.rs` next to the only consumer. There used to be
// a `#[allow(dead_code)]` second copy here that was never wired into the manda
...
///
/// The file is located in the same directory as the user's MANDA config,
/// typically `~/.config/manda/assistant.toml` on macOS/Linux.
///
/// # Errors
...
#                     MANDA skips auto-fetching from /models and cycles only\n\
...
#   Configure via `manda ai` instead of editing this file directly.\n\
...
mod tests {
    ...
    // detect_provider* tests live in `manda-gui/src/ai_client.rs` next to the
