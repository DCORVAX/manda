...
impl DoctorCommand {
    pub fn run(&self) -> anyhow::Result<()> {
        ...
        if self.prompt_fix && should_offer_auto_fix(&report) {
            match prompt_yes_no("Run safe auto-fix now with `manda init --update-only`? [Y/n] ") {
...
fn run_auto_fix_and_rerun_report(shell: Option<ManagedShell>) {
    println!("Auto-fix: running `manda init --update-only`");
...
fn build_health_group(overall_status: DoctorStatus, summary: &DoctorSummary) -> DoctorGroup {
    let mut details = vec![
        ...
        format!("MANDA version: {}", doctor_version_string()),
    ];

    if summary.fail > 0 || summary.warn > 0 {
        details
            .push("Run `manda init --update-only` after fixing shell or PATH issues".to_string());
    }
    ...
    let checks = vec![DoctorCheck {
        ...
        summary: match overall_status {
            ...
            DoctorStatus::Warn => "MANDA works but setup is incomplete".to_string(),
            DoctorStatus::Fail => "MANDA command entry is broken or missing".to_string(),
            DoctorStatus::Info => "Informational only".to_string(),
        },
        details,
        fix: if overall_status.severity_rank() >= DoctorStatus::Warn.severity_rank() {
            Some("manda init --update-only".to_string())
        } else {
            None
        },
    }];
...
fn init_update_command(shell: Option<ManagedShell>) -> String {
    match shell {
        Some(shell) => format!("manda init --update-only --shell {}", shell.name()),
        None => "manda init --update-only".to_string(),
    }
}
...
fn build_environment_group(
    ...
    let mut shell_details = vec![
        "MANDA shell integration supports zsh and fish for PATH injection and managed shell config"
            .to_string(),
    ...
    checks.push(DoctorCheck {
        ...
        summary: match (selected_shell, &shell) {
            ...
            (None, Some(value)) if shell_supported => {
                ...
                // so when they disagree (e.g. chsh to bash after `manda init`).
        ...
        fix: if !shell_supported {
            Some(
                "Use zsh or fish, or add the manda bin dir to your shell PATH manually".to_string(),
            )
        } else {
            None
        },
    ...
    if shell_kind.is_managed() {
        ...
        checks.push(DoctorCheck {
            title: "PATH Contains MANDA Managed Bin",
            ...
            details: vec![
                format!("MANDA command wrapper is expected at {}/manda", bin_dir_display),
                "This PATH entry is normally added by MANDA shell integration on startup".to_string(),
        ...
        if *shell_kind == ShellKind::Fish {
            ...
            checks.push(DoctorCheck {
                ...
                details: vec![format!(
                    "Fish loads MANDA integration via {}",
                    conf_d.display()
                )],
                fix: None,
            });
        ...
        checks.push(DoctorCheck {
            title: "PATH Contains MANDA Managed Bin",
            status: DoctorStatus::Info,
            summary: "Current shell is not managed by MANDA; skipping shell-specific PATH check"
                .to_string(),
            details: vec![
                "MANDA shell integration manages PATH for zsh and fish.".to_string(),
                "For other shells, add the manda bin directory to PATH manually.".to_string(),
            ],
            fix: None,
        });
    }
...
fn wrapper_check(
    ...
    DoctorCheck {
        ...
        fix: if exists && exec {
            ...
            Some("Run `manda init --update-only`".to_string())
        },
    }
}
...
fn build_shell_integration_group(
    ...
    if !shell_kind.is_managed() {
        checks.push(DoctorCheck {
            ...
            status: DoctorStatus::Info,
            summary: format!(
                "Current shell ({}) is not managed by MANDA; shell-specific checks skipped",
                shell_kind.name()
            ),
            details: vec!["MANDA shell integration supports zsh and fish.".to_string()],
            fix: None,
        });
        return DoctorGroup {
            title: "Shell Integration",
    ...
    checks.push(DoctorCheck {
        ...
        details: vec!["MANDA writes PATH and shell integration to this managed file".to_string()],
        fix: if init_exists {
            None
        } else {
    ...
    checks.push(wrapper_check(
        "MANDA Wrapper Script",
        managed_wrapper_path(shell_kind),
        DoctorStatus::Fail,
        vec![
            "The `manda` shell command is provided by this wrapper".to_string(),
            "Wrapper is generated by `manda init` before shell setup runs".to_string(),
        ],
    ));
    ...
    checks.push(wrapper_check(
        ...
        vec![
            ...
            "Created alongside the manda wrapper by `manda init`".to_string(),
        ],
    ));
    ...
    if is_fish {
        ...
        let mut details = vec![
            ...
            "Fish loads MANDA integration via this conf.d file on startup".to_string(),
        ];
        ...
        {
            details.push(
                "Expected an active line that sources ~/.config/manda/fish/manda.fish".to_string(),
            );
        }
        checks.push(DoctorCheck {
            title: "fish conf.d Sources MANDA Init",
            ...
            summary: if let Some(err) = &source_check.read_error {
                ...
                format!("Found valid MANDA source entry in {}", conf_d.display())
            } else {
                format!("No valid MANDA source entry in {}", conf_d.display())
            },
            details,
        ...
        checks.push(DoctorCheck {
            title: "zshrc Sources MANDA Init",
            status: if source_check.read_error.is_some() {
                DoctorStatus::Fail
            ...
            summary: if let Some(err) = &source_check.read_error {
                ...
                format!(
                    "Found {} active MANDA source line(s), including {} legacy guarded line(s) in {}",
                ...
                format!(
                    "Found {} active MANDA source line(s) in {}",
                    source_check.guarded_active_lines + source_check.unguarded_active_lines,
                    zshrc.display()
                )
            } else {
                format!("No active MANDA source line in {}", zshrc.display())
            },
            details: source_check.details(&zshrc),
            fix: if source_check.read_error.is_some() {
                Some(format!(
                    "Fix permissions or path access for {} then run `manda doctor` again",
                    zshrc.display()
                ))
            } else if source_check.has_active_lines() && !source_check.has_legacy_guarded_lines() {
                None
...
fn build_runtime_group(shell_kind: &ShellKind) -> DoctorGroup {
    ...
    let candidates = manda_bin_candidates();
    let existing = candidates
        .iter()
        .find(|p| config::is_executable_file(p))
        .cloned();
    checks.push(DoctorCheck {
        title: "MANDA App Binary",
        ...
        summary: match &existing {
            ...
            None => "MANDA CLI binary not found in known locations".to_string(),
        },
        details: candidates
            .iter()
        ...
        fix: if existing.is_some() {
            ...
            Some("Install Manda.app to /Applications or ~/Applications".to_string())
        },
    });
...
fn build_local_network_check() -> DoctorCheck {
    let app_bundle = manda_bin_candidates()
        .into_iter()
    ...
    let mut details = vec![
        "If LAN access works in Terminal or iTerm2 but fails in MANDA, compare the two launch contexts before changing shell or PATH setup.".to_string(),
        "Run these in both apps: `route -n get <ip>`, `netstat -rn | grep <subnet>`, `ifconfig`, `scutil --nwi`, `ping -v <ip>`, `nc -vz <ip> 22`.".to_string(),
        "Check macOS System Settings > Privacy & Security > Local Network and confirm MANDA is allowed.".to_string(),
        "Compare launching MANDA from Finder/Dock versus Terminal, for example `open -na /Applications/Manda.app`.".to_string(),
    ];
    ...
    if let Some(bundle) = app_bundle {
        ...
        details.push(
            "No installed Manda.app bundle was detected in the standard locations.".to_string(),
        );
    }
    ...
    DoctorCheck {
        ...
        status: DoctorStatus::Info,
        summary: "Use this when local-network access differs between MANDA and other terminals"
            .to_string(),
        details,
        fix: None,
    }
...
fn zsh_autosuggest_provider_marker(provider: &str) -> String {
    format!(r#"typeset -g _manda_autosuggest_cli_provider="{provider}""#)
}
...
fn zsh_init_loads_bundled_autosuggestions(content: &str) -> bool {
    content
        .contains(r#"source "$MANDA_ZSH_DIR/plugins/zsh-autosuggestions/zsh-autosuggestions.zsh""#)
}
...
fn build_zsh_external_autosuggest_check(init_file: &Path, provider: &str) -> DoctorCheck {
    let fix = format!(
        "Run `manda init --update-only` to regenerate {} with {} autosuggest compatibility, then restart zsh with `exec zsh -l`",
        init_file.display(),
        provider
    );
    let mut details = vec![
        ...
        "When compatibility is active, MANDA keeps Tab bound but leaves it in completion-only mode to avoid widget recursion.".to_string(),
    ];
...
impl ZshrcSourceCheck {
    ...
    fn details(&self, zshrc: &Path) -> Vec<String> {
        ...
        if !self.has_active_lines() {
            details.push(
                "Expected an active line that sources ~/.config/manda/zsh/manda.zsh".to_string(),
            );
        } else {
            details.push(format!(
        ...
        if self.has_legacy_guarded_lines() {
            details.push(
                "Found older MANDA-specific guarded source line variants; `manda init --update-only` will normalize them."
                    .to_string(),
            );
        }
        if self.malformed_escaped_path_lines > 0 {
            details.push(format!(
                "Found {} malformed MANDA source line(s) with escaped absolute paths (for example, \"\\/Users/...\"), which prevents loading manda.zsh",
                self.malformed_escaped_path_lines
            ));
        }
        if self.commented_example {
            details.push("Found a commented MANDA source line".to_string());
        }
        details
    }
}
...
fn check_zshrc_source_line(zshrc: &Path) -> ZshrcSourceCheck {
    ...
    for line in content.lines() {
        ...
        if trimmed.starts_with('#') && trimmed.contains("manda/zsh/manda.zsh") {
            result.commented_example = true;
            continue;
        }
        if is_malformed_escaped_manda_source_line(trimmed) {
            result.malformed_escaped_path_lines += 1;
            continue;
        }
        if is_active_manda_source_line(trimmed) {
            if is_legacy_guarded_manda_source_line(trimmed) {
...
fn check_fish_conf_d_source_line(path: &Path) -> FishConfSourceCheck {
    ...
    let mut has_manda_init_var = false;
    let mut sources_manda_init_var = false;
    ...
    for line in content.lines() {
        ...
        if trimmed.contains("manda/fish/manda.fish") && contains_source_command(trimmed) {
            result.has_valid_source = true;
            break;
        }

        if trimmed.contains("_manda_fish_init") && trimmed.contains("manda/fish/manda.fish") {
            has_manda_init_var = true;
        }

        if contains_source_command(trimmed) && trimmed.contains("_manda_fish_init") {
            sources_manda_init_var = true;
        }
    }

    if !result.has_valid_source && has_manda_init_var && sources_manda_init_var {
        result.has_valid_source = true;
    }
...
fn is_active_manda_source_line(trimmed_line: &str) -> bool {
    if trimmed_line.starts_with('#') || !trimmed_line.contains("manda/zsh/manda.zsh") {
        return false;
    }
    contains_source_command(trimmed_line)
}

fn is_legacy_guarded_manda_source_line(trimmed_line: &str) -> bool {
    if !is_active_manda_source_line(trimmed_line) {
        return false;
    }
    ...
    let has_manda_comparison = compact.contains("==\"manda\"")
        || compact.contains("=='manda'")
        || compact.contains("==manda")
        || compact.contains("=\"manda\"")
        || compact.contains("='manda'")
        || compact.contains("=manda");
    let has_term_guard =
        (compact.contains("${term") || compact.contains("$term")) && has_manda_comparison;
    let has_term_program_guard = compact.contains("term_program") && has_manda_comparison;

    compact.contains("wezterm_pane") || has_term_program_guard || has_term_guard
}

fn is_malformed_escaped_manda_source_line(trimmed_line: &str) -> bool {
    if trimmed_line.starts_with('#') || !trimmed_line.contains("manda/zsh/manda.zsh") {
        return false;
    }
...
fn probe_wrapper(wrapper: &Path) -> DoctorCheck {
    ...
    if !wrapper.is_file() {
        return wrapper_check(
            ...
            vec!["Generate the wrapper first with `manda init`".to_string()],
            Some("Run `manda init --update-only`".to_string()),
        );
    }
        ...
        .spawn()
    {
        ...
        Err(err) => {
            return wrapper_check(
                ...
                Some("Restore wrapper permissions or rerun `manda init --update-only`".to_string()),
            );
        }
    };
        ...
        match child.try_wait() {
            ...
            Ok(None) => {
                ...
                return wrapper_check(
                    ...
                    Some(
                        "Check that the manda binary is accessible and not blocked by network or permission issues".to_string(),
                    ),
                );
            }
    ...
    match output {
        Ok(output) if output.status.success() => {
            ...
            wrapper_check(
                DoctorStatus::Ok,
                "Wrapper can launch MANDA binary".to_string(),
                if stdout.is_empty() {
                    vec![format!(
        ...
        Ok(output) => {
            ...
            wrapper_check(
                ...
                details,
                Some("Check Manda.app location then run `manda init --update-only`".to_string()),
            )
        }
...
fn probe_login_shell_integration(shell_kind: &ShellKind) -> DoctorCheck {
    let (title, detail_exec, detail_verify) = match shell_kind {
        ShellKind::Fish => (
            ...
            "Start a new fish session and verify `echo $PATH` includes ~/.config/manda/fish/bin if you need end-to-end runtime validation.",
        ),
        ShellKind::Zsh => (
            ...
            "Use `exec zsh -l` manually and verify `echo $MANDA_ZSH_DIR` if you need end-to-end runtime validation.",
        ),
        _ => (
            "Login Shell Integration Probe",
            "Doctor skips login shell probe for shells not managed by MANDA.",
            "MANDA shell integration supports zsh and fish. Other shells are not managed.",
        ),
    };
...
fn managed_wrapper_path(shell_kind: &ShellKind) -> PathBuf {
    managed_bin_dir(shell_kind).join("manda")
}
...
fn manda_bin_candidates() -> Vec<PathBuf> {
...
mod tests {
    ...
    fn variable_assignment_is_not_detected_as_source_line() {
        ...
        fs::write(
            &path,
            r#"export MANDA_ZSH_INIT="$HOME/.config/manda/zsh/manda.zsh""#,
        )
        .expect("write zshrc");
    ...
    fn counts_guarded_and_unguarded_source_lines() {
        ...
        fs::write(
            &path,
            r#"[[ "${TERM:-}" == "manda" ]] && source "$HOME/.config/manda/zsh/manda.zsh"
[[ -f "$HOME/.config/manda/zsh/manda.zsh" ]] && source "$HOME/.config/manda/zsh/manda.zsh"
[[ -f "$HOME/.config/manda/zsh/manda.zsh" ]] && source "$HOME/.config/manda/zsh/manda.zsh"
"#,
        )
        .expect("write zshrc");
    ...
    fn term_program_guard_is_detected_as_legacy_source_line() {
        ...
        fs::write(
            &path,
            r#"[[ "$TERM_PROGRAM" == "MANDA" ]] && [[ -f "$HOME/.config/manda/zsh/manda.zsh" ]] && source "$HOME/.config/manda/zsh/manda.zsh" # MANDA Shell Integration
"#,
        )
        .expect("write zshrc");
    ...
    fn escaped_absolute_path_source_line_is_marked_malformed() {
        ...
        fs::write(
            &path,
            r#"[[ -f "\/Users/lex/.config/manda/zsh/manda.zsh" ]] && source "\/Users/lex/.config/manda/zsh/manda.zsh""#,
        )
        .expect("write zshrc");
    ...
    fn explicit_shell_is_preserved_in_auto_fix_command() {
        assert_eq!(
            ...
            "manda init --update-only --shell fish"
        );
        assert_eq!(init_update_command(None), "manda init --update-only");
    }

    #[test]
    fn fish_conf_d_missing_is_not_valid_source() {
        let dir = tempfile::tempdir().expect("create tempdir");
        let path = dir.path().join("manda.fish");
    ...
    #[test]
    fn fish_conf_d_without_manda_source_is_not_valid() {
        let dir = tempfile::tempdir().expect("create tempdir");
        let path = dir.path().join("manda.fish");
    ...
    fn fish_conf_d_with_valid_source_is_detected() {
        let dir = tempfile::tempdir().expect("create tempdir");
        let path = dir.path().join("manda.fish");
        fs::write(&path, "source ~/.config/manda/fish/manda.fish\n").expect("write");
        let check = check_fish_conf_d_source_line(&path);
        assert!(check.has_valid_source);
        assert!(!check.missing_file);
        assert!(check.read_error.is_none());
    ...
    fn fish_conf_d_with_managed_variable_source_is_detected() {
        ...
        fs::write(
            &path,
            r#"# MANDA shell integration -- managed. Remove with: manda reset
...
"#;
    ...
    fn autosuggest_compatibility_rejects_bundled_source_line() {
        let content = r#"
typeset -g _manda_autosuggest_cli_provider="kiro-cli"
