...
const MANDA_AUTO_COLOR_SCHEME_EXPR: &str = "(wezterm.gui and wezterm.gui.get_appearance() or 'Dark'):find('Dark') and 'MANDA Dark' or 'MANDA Light'";
...
impl App {
    fn new(config_path: PathBuf) -> Self {
        let fields = vec![
            ...
            ConfigField {
                ...
                value: String::new(),
                default: "Auto".into(),
                options: vec!["Auto", "MANDA Dark", "MANDA Light"],
                skip_write: false,
            },
            ConfigField {
                section: "Appearance",
    ...
    fn extract_lua_value(content: &str, key: &str) -> Option<String> {
        ...
        for line in content.lines() {
            ...
            if key == "color_scheme" && Self::is_manda_auto_color_scheme_expr(&value) {
    ...
    fn is_manda_auto_color_scheme_expr(raw: &str) -> bool {
        raw.trim() == MANDA_AUTO_COLOR_SCHEME_EXPR
    }
    ...
    /// Also signals manda-gui immediately after a successful write so it reloads
    ...
    fn save_if_dirty(&mut self) -> anyhow::Result<()> {
        if self.dirty {
            ...
            // manda-gui. Sending after LeaveAlternateScreen is unreliable because
    ...
    fn save_config(&self) -> anyhow::Result<()> {
        ...
        for field in &self.fields {
            ...
            // manda.lua is a single-file config (not an overlay on bundled
        ...
        if let (Some(tl), Some(sh)) = (tl_field, sh_field) {
            if !tl.skip_write || !sh.skip_write {
                ...
                // manda.lua is the single source of truth and the line may
    ...
    fn strip_managed_theme_blocks(content: &str) -> String {
        let (content, _) =
            Self::strip_theme_block(content, "-- ===== MANDA Theme Defaults (managed) =====");
        let (content, _) = Self::strip_theme_block(&content, "-- ===== MANDA Theme =====");
        content
    }
    ...
    fn strip_theme_block(content: &str, marker: &str) -> (String, bool) {
        ...
        // Older MANDA versions appended managed colors/window_frame just before
    ...
    fn to_lua_value(&self, field: &ConfigField) -> String {
        match field.lua_key {
            "color_scheme" => {
                if field.value == "Auto" {
                    MANDA_AUTO_COLOR_SCHEME_EXPR.into()
                } else {
                    format!("'{}'", field.value)
                }
            }
...
/// Send an OSC 1337 SetUserVar to signal manda-gui that config has changed.
...
fn signal_config_changed() {
    ...
    // name: MANDA_CONFIG_CHANGED, value: "1" -> base64 "MQ=="
    let seq = if std::env::var("TMUX").is_ok() {
        // tmux passthrough: wrap OSC in DCS tmux; ... ST
        b"\x1bPtmux;\x1b\x1b]1337;SetUserVar=MANDA_CONFIG_CHANGED=MQ==\x07\x1b\\" as &[u8]
    } else {
        b"\x1b]1337;SetUserVar=MANDA_CONFIG_CHANGED=MQ==\x07" as &[u8]
    };
...
mod tests {
    ...
    fn test_app() -> App {
        App::new(PathBuf::from("/tmp/manda-config-tui-test.lua"))
    }
    ...
    fn color_scheme_update_strips_legacy_managed_theme_block() {
        ...
        let idx = app
            ...
            .position(|f| f.lua_key == "color_scheme")
            .expect("color_scheme field to exist");
        app.fields[idx].value = "MANDA Light".to_string();
...
";
        ...
        assert!(!updated.contains("-- ===== MANDA Theme ====="));
        assert!(!updated.contains("config.colors"));
        assert!(!updated.contains("config.window_frame"));
        assert!(updated.contains("config.color_scheme = 'MANDA Light'"));
...
";
    ...
    fn load_config_round_trips_serialized_auto_theme_expression() {
        ...
        let config_path = dir.path().join("manda.lua");
        std::fs::write(
            &config_path,
            format!("config.color_scheme = {MANDA_AUTO_COLOR_SCHEME_EXPR}\n"),
        )
        .expect("write config");
    ...
    fn dynamic_color_scheme_expression_is_not_parsed_as_writable_value() {
        let content =
            "config.color_scheme = appearance == 'Dark' and 'MANDA Dark' or 'MANDA Light'\n";
    ...
    fn ensure_editable_config_creates_missing_custom_path() {
        let dir = tempdir().expect("tempdir");
        let config_path = dir.path().join("nested").join("custom-manda.lua");
    ...
    fn selecting_esc_confirms_and_marks_dirty() {
        ...
        assert_eq!(app.fields[idx].value, "MANDA Dark");
    }
    ...
    fn window_decorations_default_state_preserves_explicit_line() {
        ...
        // because manda.lua is the single source of truth; removing it
