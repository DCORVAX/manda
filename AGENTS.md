...
MANDA is a macOS-native terminal emulator derived from WezTerm and shaped around AI-assisted terminal workflows. This guide is the shared operating context for agents working in this repository.

## Repository Map

- `manda/` - CLI entry points, command flows, and user-facing configuration commands.
- `manda-gui/` - GUI, rendering, window lifecycle, input, mouse handling, AI chat, and the `k` helper binary.
...
- `crates/` - shared utility crates, including MANDA-specific AI helpers.
...
- The marketing site lives in `web/` inside this repository: `index.html` (EN home), `features.html`, `shortcuts.html`, plus Spanish mirrors under `web/es/` — sharing `web/style.css` (FlipTrack-inspired warm monochrome design system; light primary, dark secondary via theme toggle) and `web/script.js`. It deploys to `manda-term.vercel.app`; point Vercel at the `web/` directory as the project root. Do not add instructions for the removed `website/` tree.
...
- The rolling `nightly` release is not rebuilt by push. Before sending users to Nightly, run or verify `./scripts/nightly.sh` and confirm `gh release view nightly --json tagName,targetCommitish,publishedAt,assets,url` points at the fix commit and includes `MANDA-nightly.dmg`.
...
2. Entry point for the behavior, usually `manda/src/main.rs`, `manda/src/cli/`, or `manda-gui/src/main.rs`.
...
1. CLI and assistant configuration under `manda/src/ai_config/`, `manda/src/assistant_config.rs`, and `config/src/proxy.rs`.
2. GUI AI state and transport under `manda-gui/src/ai_*`, `manda-gui/src/ai_chat_engine/`, and `manda-gui/src/cli_chat/`.
3. Overlay UI under `manda-gui/src/overlay/ai_chat/`.
4. Shared helpers in `crates/manda-ai-utils/`.
...
1. AppKit menu `keyEquivalent` intercepting `keyDown` before the terminal sees it. Enable `config.debug_key_events = true`, restart the app, then `grep 'key_event.*CTRL' ~/.local/share/manda/manda-gui-log-<pid>.txt`. If the log shows only `key_is_down: false` and no matching `key_is_down: true`, the AppKit menu absorbed the event; do not chase termwiz or PTY.
...
|---|---|---|
| GUI | `manda-gui/AGENTS.md` | Rendering, window lifecycle, input, mouse |
...
| `crates/manda-relay` change | `cargo check --locked --manifest-path crates/manda-relay/Cargo.toml` (it is in `workspace.exclude`, so `make check` and `make test` never see it; CI's `relay-check` job is the only gate) |

For GUI or rendering issues, read `manda-gui/AGENTS.md` first and verify with `make app`, not only `make dev`.

## Current Risk Areas

- In-app update replace/restart must confirm on every entry path (menu Check for Updates, menu Restart to Update, toast, CLI direct ZIP, brew cask). Do not set `MANDA_UPDATE_AUTO_CONFIRM` for exploratory menu check; only the overlay-confirmed path may auto-confirm. Sibling entry points tend to regress independently; after changing one path, sweep the matrix and keep the guards in `manda-gui` (`user_facing_update_events_route_through_confirm`) and `manda` (`brew_and_direct_paths_confirm_before_replace`) green. Details: `manda-gui/AGENTS.md` and `.agents/skills/bugs/SKILL.md`.
...
- Notification actions that call back into MANDA should resolve bundled executables relative to the running app, not an assumed system path.
- Known high-regression zones: theme/config TUI initialization (`manda/src/ai_config/tui/`), macOS window geometry (`window/src/os/macos/window.rs`, `manda-gui/src/termwindow/resize.rs`), and menubar initialization timing (`window/src/os/macos/menu.rs`). Changes touching these must ship with a regression test or assertion.
- `assets/shell-integration` scripts run in the user's shell, not just at build time. Bash heredocs that generate zsh (e.g. `setup_zsh.sh` writing `manda.zsh`) expand backticks and `$(...)` at generation time, so escape any that must reach the output literally (#450), and never put `local` outside a function (#432/#441). CI gates this: shellcheck (`--severity=error`, catches SC2168) over the bash scripts plus a `zsh -n` parse check of the generated `manda.zsh` in the setup smoke.
