...
MANDA auto-creates `~/.config/manda/manda.lua` with a commented template on first launch. Open it with `manda config` or `Cmd + ,`.
...
local function resolve_bundled_config()
  ...
  return '/Applications/Manda.app/Contents/Resources/manda.lua'
end
...
> The full boilerplate with all available commented examples is auto-generated by `manda init`. Most users only need to uncomment the lines they want to change.
...
MANDA follows macOS appearance by default and switches between MANDA Dark and
MANDA Light automatically. To return to this behavior, choose **Auto** in
`manda config` or remove an explicit `config.color_scheme` override. To force
one theme:

```lua
config.color_scheme = "MANDA Dark"   -- always dark
config.color_scheme = "MANDA Light"  -- always light
```
...
MANDA auto-selects 15px (low-res) or 17px (high-res) based on your display. Override:
...
By default, MANDA embeds the macOS traffic light buttons into the tab bar area using `INTEGRATED_BUTTONS|RESIZE`. To hide the traffic lights while keeping resize edges and tab-bar dragging:
...
Set an editor command for local file links printed in the terminal. MANDA appends
...
This setting takes priority over MANDA's automatic VS Code detection. When it is
unset, MANDA keeps the existing VS Code and `$VISUAL` / `$EDITOR` fallback behavior.
...
Disabled by default. Enable via `manda config` (toggle the scrollbar style option) or in Lua:
...
nano and vim, instead of peeking into MANDA's primary scrollback, enable:
...
with the left mouse button held down. Defaults to `"Extend"` (MANDA v0.11+),
...
-- Drop the wheel event entirely. This is the legacy MANDA v0.10 behavior;
...
> **Default change in v0.11**: earlier MANDA versions behaved as if
...
MANDA checks GitHub for new releases in the background (enabled by default) and
...
You can always update manually with `manda update` or from the app menu,
...
Always **insert** into `config.keys`, never replace it. Replacing erases all MANDA defaults.
...
Add custom HTTP headers to MANDA Assistant API requests (for corporate proxies or API gateways):
