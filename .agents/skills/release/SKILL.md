---
name: release
description: "Run the MANDA macOS release flow end-to-end: preflight, build, notarize, tag, upload to GitHub Releases, dispatch the Homebrew tap, or publish the rolling Nightly preview package. Source of truth is scripts/release.sh and scripts/nightly.sh; this skill is a runbook with one-time setup commands and recovery hints."
when_to_use: "发布, 出版本, 打 tag, release MANDA, ship MANDA, V0.x.x, manda release, prepare release, dry-run release, resume release, notarize, homebrew tap, nightly, Nightly 包, 更新 nightly, 预览包"
---

# MANDA Release Runbook
...
MANDA_ASC_API_KEY_PATH=/dev/null ./scripts/release.sh        # full release on slow network (recommended)
...
Tang's credentials live at `~/save/AppleCertificates/`. The release script checks for either `manda-asc-api-key-path` (rcodesign route, preferred) or `manda-notarytool-profile` (notarytool fallback) in the login keychain.
...
security add-generic-password -s 'manda-asc-api-key-path' -a 'manda' \
...
```bash
xcrun notarytool store-credentials manda-notarytool \
...
security add-generic-password -s 'manda-notarytool-profile' -a 'manda' -w 'manda-notarytool'
```
...
```bash
which rcodesign
security find-generic-password -s 'manda-asc-api-key-path' -w
...
1. `manda/Cargo.toml` and `manda-gui/Cargo.toml` versions match (e.g. `0.10.0`).
...
These are the areas that produce the most post-release bug reports and that CI cannot see (visual layout, native AppKit, shell-in-user-env). Run them by hand in the built `dist/MANDA.app` before tagging. Automated coverage already exists for the testable slices: tab width-budget + hover hit-testing have unit tests in `manda-gui/src/tabbar.rs`, and `local`-outside-function / shell syntax is gated by shellcheck + the setup_zsh smoke. The list below is what still needs a human.
...
3. **Shell setup** (#420, #432, #441, #450): from a clean `HOME`, run `manda init` and confirm z / syntax-highlight / autosuggestions are active in a fresh shell; run `manda init --update-only` and confirm it exits clean; open a new shell and confirm `~/.config/manda/zsh/manda.zsh` sources with no error.
4. **AI chat** (#418, #431): run `manda chat`, quit, then run it again in the same window and confirm it reopens.
...
3. **stage:build** — `./scripts/build.sh`, `PROFILE=release-opt`, `BUILD_ARCH=universal`. Output: `dist/MANDA.app`, `dist/MANDA.dmg`, `dist/manda_for_update.zip`, `dist/manda_for_update.zip.sha256`.
...
7. **stage:homebrew-tap** — `repository_dispatch` to `WILFREDY-X/homebrew-tap` triggering `bump.yml`. Polls the cask file in `Casks/manda.rb` for the new version (default 12 attempts × 15s).
...
| `--upload-only` | build, notarize | Notarized `dist/MANDA.app` and `MANDA.dmg` exist on disk; only need to tag + upload + tap. |
...
- **`No Developer ID Application certificate found`**: re-import `~/save/AppleCertificates/MANDA_DevID.p12` (password in `p12_password.txt`) into the login keychain.
...
|---|---|---|
| `MANDA_SIGNING_IDENTITY` | auto-detect | Override the Developer ID Application identity. |
| `MANDA_ASC_API_KEY_PATH` | keychain item `manda-asc-api-key-path` | Path to rcodesign ASC API key JSON. |
| `MANDA_NOTARYTOOL_PROFILE` | keychain item `manda-notarytool-profile` | notarytool keychain profile name. |
