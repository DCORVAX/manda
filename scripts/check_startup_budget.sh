...
#
# Read a JSON report from scripts/measure_startup_manda.sh and check the
...
#
# Budgets are derived from the hyperfine *mean* (see measure_startup_manda.sh),
...
if [[ "$cold_max" -eq 0 || "$warm_max" -eq 0 ]]; then
  echo "WARN: budget is 0 (placeholder). Run scripts/measure_startup_manda.sh"
