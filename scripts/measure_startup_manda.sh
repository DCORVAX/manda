#!/usr/bin/env bash
# scripts/measure_startup_manda.sh
#
# Measure Manda.app cold-start time and emit a JSON report compatible with
...
APP_PATH="${APP_PATH:-dist/Manda.app}"
...
quit_manda() {
  pkill -9 -x "manda-gui" >/dev/null 2>&1 || true
  for _ in {1..200}; do
    pgrep -x "manda-gui" >/dev/null 2>&1 || return 0
    sleep 0.05
  done
}
...
tell application "System Events"
  repeat
    ...
    if ((current date) - startAt) > timeoutSeconds then
      error "timeout waiting for MANDA window"
    end if
    delay 0.05
  end repeat
end tell
...
cold_cmd="$(declare -f quit_manda); $(declare -f wait_first_window); quit_manda >/dev/null 2>&1; open -a '$APP_PATH'; wait_first_window $WAIT_TIMEOUT_SEC"
...
warm_cmd="open -a '$APP_PATH'; $(declare -f wait_first_window); wait_first_window $WAIT_TIMEOUT_SEC; pkill -9 -x manda-gui >/dev/null 2>&1 || true"
hyperfine \
  --warmup 2 \
  --runs "$RUNS" \
  --export-json "$tmp_json" \
