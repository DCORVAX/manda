...
- AI HTTP transport must preserve both sides of proxy behavior: external API hosts should honor detected system proxy settings, while loopback, private LAN, link-local, CGNAT/Tailscale-style, `.local`, `NO_PROXY`, and macOS ExceptionsList model endpoints should bypass the proxy and connect directly. Do not describe this as full SOCKS support unless reqwest SOCKS support is actually enabled and smoke-tested. Exception: the `http_request` tool pins validated IPs against DNS-rebinding SSRF, so it routes through the system proxy only for https (TLS still authenticates the origin through CONNECT); plaintext http stays direct and IP-pinned. Residual, accepted boundary: a split-DNS corporate proxy can re-resolve an approved https hostname to an internal host holding a valid certificate for that name; the per-request approval prompt (all methods, including GET) is the guard for that case.
