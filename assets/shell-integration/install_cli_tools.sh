...
case "${MANDA_TARGET_SHELL:-${SHELL:-/bin/zsh}}" in
*fish|fish)
	USER_BIN_DIR="$HOME/.config/manda/fish/bin"
	RELOAD_CMD="exec fish"
	;;
*)
	USER_BIN_DIR="$HOME/.config/manda/zsh/bin"
	RELOAD_CMD="exec zsh"
	;;
esac
MISSING_TOOLS=()
...
prompt_install_homebrew() {
	...
	echo ""
	echo "Homebrew is required to install MANDA's optional CLI tools:"
...
ensure_homebrew_installation() {
	...
	echo "MANDA shell integration will continue without them."
	echo "Install Homebrew later, then refresh optional tools with:"
	echo "  manda init --update-only"
