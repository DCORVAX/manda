...
# Remote machines typically don't have the "manda" terminfo entry, so when
# SSHing from a MANDA local shell (TERM=manda), zsh plugins on the remote side
...
# Override TERM for ssh sessions the same way MANDA's built-in SSH domain does.
# Set MANDA_SSH_SKIP_TERM_FIX=1 to disable. If the user already defined ssh(),
...
if [ -n "${ZSH_NAME-}" ] && alias ssh >/dev/null 2>&1; then
  _manda_existing_ssh_alias="${aliases[ssh]-}"
  function ssh {
    local -a _manda_alias_words _manda_ssh_cmd
    ...
    if [[ ${#_manda_alias_words[@]} -eq 0 ]]; then
      ...
      _manda_ssh_cmd=(command ssh "${(@)_manda_alias_words[2,-1]}")
    elif [[ "${_manda_alias_words[1]-}" == "command" && "${_manda_alias_words[2]-}" == "ssh" ]]; then
      _manda_ssh_cmd=(command ssh "${(@)_manda_alias_words[3,-1]}")
    ...
    if [ -z "${MANDA_SSH_SKIP_1PASSWORD_FIX-}" ]; then
      case "${SSH_AUTH_SOCK-}" in
        *1password*|*2BUA8C4S2C*)
          case " $* " in
            *IdentitiesOnly=*) ;;
            *) _manda_1p_opt="-oIdentitiesOnly=yes" ;;
          esac
          ;;
      esac
    fi
    if [[ -z "${MANDA_SSH_SKIP_TERM_FIX-}" && "${TERM:-}" == "manda" ]]; then
      TERM=xterm-256color "${_manda_ssh_cmd[@]}" ${_manda_1p_opt:+"$_manda_1p_opt"} "$@"
    else
      "${_manda_ssh_cmd[@]}" ${_manda_1p_opt:+"$_manda_1p_opt"} "$@"
    fi
  }
  unalias ssh
  ...
  function ssh {
    ...
    if [[ -z "${MANDA_SSH_SKIP_TERM_FIX-}" && "${TERM:-}" == "manda" ]]; then
      TERM=xterm-256color command ssh ${_manda_1p_opt:+"$_manda_1p_opt"} "$@"
    else
      command ssh ${_manda_1p_opt:+"$_manda_1p_opt"} "$@"
    fi
  }
fi
...
# Check for available MANDA updates on shell startup
__manda_check_update_notification() {
  # macOS uses ~/Library/Application Support, Linux uses XDG_DATA_HOME
  local check_file
  if [[ "$OSTYPE" == darwin* ]]; then
    check_file="$HOME/Library/Application Support/manda/check_update"
  else
    check_file="${XDG_DATA_HOME:-$HOME/.local/share}/manda/check_update"
  fi
  ...
  if command -v manda >/dev/null 2>&1; then
    current_version=$(manda --version 2>/dev/null | head -1 | awk '{print $2}')
  fi
  ...
  local data_dir
  if [[ "$OSTYPE" == darwin* ]]; then
    data_dir="$HOME/Library/Application Support/manda"
  else
    data_dir="${XDG_DATA_HOME:-$HOME/.local/share}/manda"
  fi
  ...
  if [[ "$newer" == "$latest_clean" && "$newer" != "$current_clean" ]]; then
    local key
    printf '\033[1;36mMANDA %s is available. Press Enter to update, any other key to skip.\033[0m ' "$latest_clean"
