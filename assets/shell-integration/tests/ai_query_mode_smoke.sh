...
zsh_body="$(sed -n '/_manda_ai_query_accept_line()/,/^}/p' "$zsh_script")"
fish_body="$(sed -n '/function __manda_ai_query_execute/,/^end/p' "$fish_script")"

[[ "$zsh_body" == *'mode="explain"'* ]] || fail "zsh explain mode missing"
[[ "$zsh_body" == *'mode="candidates"'* ]] || fail "zsh candidates mode missing"
[[ "$zsh_body" == *'if _manda_set_ai_user_var "manda_ai_query" "[mode:\${mode}] \${body}"; then'* ]] \
...
[[ "$fish_body" == *'if __manda_set_ai_user_var manda_ai_query "[mode:$mode] $query"'* ]] \
