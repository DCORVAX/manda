...
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/manda-kiro-autosuggest.XXXXXX")"
cleanup() {
  rm -rf "$tmp_dir"
}
trap cleanup EXIT
...
grep -Fq 'typeset -g _manda_autosuggest_cli_provider="kiro-cli"' "$manda_zsh" \
  || fail "managed init did not record kiro-cli compatibility mode"

if grep -Fq 'source "$MANDA_ZSH_DIR/plugins/zsh-autosuggestions/zsh-autosuggestions.zsh"' "$manda_zsh"; then
  fail "managed init still sources bundled zsh-autosuggestions in kiro compatibility mode"
fi

grep -Fq 'if [[ "${_manda_external_autosuggest_provider:-0}" != "1" ]] && (( ${+widgets[autosuggest-accept]} )) && [[ -n "${POSTDISPLAY:-}" ]]; then' "$manda_zsh" \
...
print -r -- "__MANDA_AUTOSUGGEST_PROVIDER__:${_manda_autosuggest_cli_provider:-}"
print -r -- "__MANDA_EXTERNAL_AUTOSUGGEST__:${_manda_external_autosuggest_provider:-0}"
' 2>&1
)"; then
  echo "$runtime_out" >&2
...
case "$runtime_out" in
  *__MANDA_AUTOSUGGEST_PROVIDER__:kiro-cli* ) ;;
  * ) echo "$runtime_out" >&2; fail "runtime provider marker was not preserved" ;;
esac

case "$runtime_out" in
  *__MANDA_EXTERNAL_AUTOSUGGEST__:1* ) ;;
