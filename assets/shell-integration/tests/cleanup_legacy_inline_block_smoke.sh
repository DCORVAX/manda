...
# known MANDA-managed line. Unknown user lines keep the block intact.
...
run_cleanup() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if [[ "$saw_manda_var" == "1" && "$saw_syntax" == "1" && "$line" =~ ^[[:space:]]*fi[[:space:]]*$ ]]; then
      ...
      if [[ "$managed" == "0" ]]; then
        printf '%s\n' "# MANDA Shell Integration" >>"$output_file"
        for block_line in "${block_lines[@]}"; do
...
run_test() {
  ...
  tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/manda-cleanup-test.XXXXXX")"
...
LEGACY_BLOCK='# MANDA Shell Integration
export MANDA_ZSH_DIR="$HOME/.config/manda/zsh"
source "$MANDA_ZSH_DIR/plugins/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh"
fi'
