...
if os.getenv('MANDA_STRICT_CONFIG') == '1' and wezterm.config_builder then
...
local function default_manda_user_config_path()
  local xdg = os.getenv('XDG_CONFIG_HOME')
  if xdg and xdg ~= '' then
    return xdg .. '/manda/manda.lua'
  end
  local home = os.getenv('HOME')
  if home then
    return home .. '/.config/manda/manda.lua'
  end
  return nil
end

local function is_bundled_manda_config_path(path)
  if type(path) ~= 'string' or path == '' then
    return false
  end

  local normalized = path:gsub('\\', '/')
  return normalized:match('/MANDA%.app/Contents/Resources/manda%.lua$') ~= nil
    or normalized:match('/assets/macos/MANDA%.app/Contents/Resources/manda%.lua$') ~= nil
end
...
local function manda_user_config_path()
  ...
  if (not runtime_config or runtime_config == '') then
    local env_config = os.getenv('MANDA_CONFIG_FILE')
    if env_config and env_config ~= '' then
      runtime_config = env_config
    end
  end

  if runtime_config and runtime_config ~= '' and not is_bundled_manda_config_path(runtime_config) then
    return runtime_config
  end

  return default_manda_user_config_path()
end
...
do
  local user_config_path = manda_user_config_path()
    ...
    for line in file:lines() do
      ...
      if trimmed and not trimmed:match('^%-%-') then
        ...
        if user_theme_selection == 'none' and trimmed:match('^config%.color_scheme%s*=') then
          if trimmed:match("^config%.color_scheme%s*=%s*['\"]MANDA Light['\"]") then
            user_theme_selection = 'light'
          elseif trimmed:match("^config%.color_scheme%s*=%s*['\"]MANDA Dark['\"]") then
...
-- `manda config` / `manda ai`). Mirrors the Rust-side is_macos_dark_mode().
...
local function resolve_appearance_color_scheme()
  ...
  if gui and type(gui.get_appearance) == 'function' then
    ...
    if ok and type(appearance) == 'string' then
      return appearance:find('Dark', 1, true) and 'MANDA Dark' or 'MANDA Light'
    end
  end
  return is_macos_dark_appearance() and 'MANDA Dark' or 'MANDA Light'
end

local function resolve_manda_color_scheme(scheme)
  if scheme == 'Auto' then
...
-- Optional Claude Code theme sync with MANDA color scheme.
-- Disabled by default because it writes ~/.claude.json.
-- Set MANDA_CLAUDE_SYNC=1 to opt in.
local manda_last_synced_cc_theme = nil

local function sync_claude_code_theme(is_light)
  if os.getenv('MANDA_CLAUDE_SYNC') ~= '1' then return end
  local target = is_light and 'light-ansi' or 'dark-ansi'
  if manda_last_synced_cc_theme == target then return end
  local home = os.getenv('HOME')
  if not home or home == '' then return end
  manda_last_synced_cc_theme = target
...
local manda_state_dir = home_dir and (home_dir .. "/.config/manda") or nil
local xdg_config_home = os.getenv("XDG_CONFIG_HOME")
local manda_config_dir = xdg_config_home and xdg_config_home ~= ""
    and (xdg_config_home .. "/manda")
  or manda_state_dir
local lazygit_state_file = manda_state_dir and (manda_state_dir .. "/lazygit_state.json") or nil
local last_cwd_file = manda_state_dir and (manda_state_dir .. "/last_cwd") or nil
...
local remote_files_mount_root = home_dir and (home_dir .. "/Library/Caches/dev.manda/sshfs") or nil
...
local ai_fix_toml_path = manda_config_dir and (manda_config_dir .. "/assistant.toml") or nil
...
local ai_fix_jobs_dir = manda_state_dir and (manda_state_dir .. "/ai_jobs") or "/tmp"
...
local function ai_debug_log(message)
  ...
  if not manda_state_dir then
    return
  end
  local dir_status = os.execute(
    string.format("mkdir -p %q && chmod 700 %q", manda_state_dir, manda_state_dir)
  )
  if dir_status ~= true and dir_status ~= 0 then
    return
  end
  local debug_path = manda_state_dir .. "/ai_debug.log"
...
-- In a built bundle the prompts live next to manda.lua under Resources/prompts/.
...
local function start_ai_fix_background_job(payload, window, pane)
  ...
  local launched_ok, launch_err = pcall(function()
    window:perform_action(
      wezterm.action.EmitEvent("manda-ai-inline-request:" .. job.id),
      pane
    )
  end)
...
local function inject_ai_notice(pane, headline, detail, suggested_command)
  ...
  local is_light = resolve_appearance_color_scheme() == 'MANDA Light'
  ...
  local summary_line = label_color .. "╭─ MANDA Assistant\27[0m  \27[1m" .. summary .. "\27[0m"
  local command_line = ""
  if cmd ~= "" then
...
local function inject_ai_status(pane, message)
  ...
  local line = label_color .. "╰─ MANDA Assistant\27[0m  " .. summary_color .. summary .. "\27[0m"
  local output = "\r\n" .. line .. "\r\n\r\n"
  pcall(function()
    pane:inject_output(output)
  end)
...
local function poll_ai_fix_job(window, pane, pane_id, job, failed_command, exit_code)
  ...
  if status_code == ai_inline_not_configured_status then
    ...
    pcall(function()
      window:perform_action(wezterm.action.EmitEvent("manda-toast-ai-clear-progress"), pane)
    end)
    return
  end
...
local function poll_ai_generate_job(window, pane, pane_id, job)
  ...
  if status_code == ai_inline_not_configured_status then
    ...
    safe_send_clear(pane)
    pcall(function()
      window:perform_action(wezterm.action.EmitEvent("manda-toast-ai-missing-key"), pane)
    end)
    return
  end
...
local function ensure_manda_state_dir()
  if not manda_state_dir or manda_state_dir == "" then
    return
  end
  os.execute(string.format("mkdir -p %q", manda_state_dir))
end
...
local function resolve_yazi_command()
  ...
  if home ~= "" then
    table.insert(candidates, 1, home .. "/.config/manda/zsh/bin/yazi")
    table.insert(candidates, 1, home .. "/.config/manda/fish/bin/yazi")
  end
  local resolved = nil
  for _, cmd in ipairs(candidates) do
    local call_ok, run_result = pcall(function()
...
local manda_yazi_theme_marker_start = "# ===== MANDA Yazi Flavor (managed) ====="
local manda_yazi_theme_marker_end = "# ===== End MANDA Yazi Flavor (managed) ====="

local function current_yazi_flavor(window)
  local overrides = window and window:get_config_overrides() or {}
  local scheme = resolve_manda_color_scheme(overrides.color_scheme or config.color_scheme)
  return scheme == 'MANDA Light' and 'manda-light' or 'manda-dark'
end
...
local function strip_managed_yazi_theme_block(content)
  ...
  for line in (content .. "\n"):gmatch("(.-)\n") do
    if line == manda_yazi_theme_marker_start then
      skipping = true
    elseif line == manda_yazi_theme_marker_end then
      skipping = false
...
local function is_legacy_manda_yazi_theme(content)
  if content:find("# MANDA%-aligned theme for Yazi 26%.x", 1) then
    return true
  end
...
local function sync_managed_yazi_theme(window)
  ...
  -- form is a `#:schema <url>` comment); older MANDA versions wrote the key,
  ...
  if not is_legacy_manda_yazi_theme(existing) and existing:find(managed_block, 1, true) then
    return
  end

  local updated
  if existing == "" or is_legacy_manda_yazi_theme(existing) then
    ...
    local has_managed = existing:find(manda_yazi_theme_marker_start, 1, true) ~= nil
...
function manda_foreground_process_title(pane)
  if not pane then
    return nil
  end
...
function manda_context_process_title(context, process)
...
local function ensure_remote_mount(sshfs_cmd, remote_target, mount_path)
  ...
  local volume_name = "MANDA-" .. sanitize_mount_component(remote_target)
...
local function open_remote_files(window, pane)
  ...
  if not yazi_cmd then
    show_remote_files_toast(window, "Yazi not found. Run manda init.")
    return
  end
...
local function maybe_show_lazygit_hint(window, pane)
  ...
  show_lazygit_toast(window, pane, "manda-toast-try-lazygit")
...
local function launch_lazygit(window, pane)
  pane = resolve_active_pane(window, pane)
  if not pane then
    show_lazygit_toast(window, pane, "manda-toast-lazygit-no-pane")
    return
  end

  local path = pane_cwd(pane)
  if path == "" then
    show_lazygit_toast(window, pane, "manda-toast-lazygit-no-cwd")
    return
  end

  local repo_root = detect_git_repo_root(path)
  if not repo_root then
    show_lazygit_toast(window, pane, "manda-toast-lazygit-not-git")
    return
  end

  local lazygit_cmd = resolve_lazygit_command()
  if not lazygit_cmd then
    show_lazygit_toast(window, pane, "manda-toast-lazygit-missing")
    return
  end
  ...
  if not ok then
    show_lazygit_toast(window, pane, "manda-toast-lazygit-dispatch-failed")
    return
  end
  mark_repo_lazygit_used(repo_root)
end
...
local function launch_yazi(window, pane)
  pane = resolve_active_pane(window, pane)
  if not pane then
    show_yazi_toast(window, pane, "manda-toast-yazi-no-pane")
    return
  end
  ...
  if not remote_target then
    ...
    if not yazi_cmd then
      ...
      show_yazi_toast(window, pane, "manda-toast-yazi-missing")
      return
    end
  ...
  if not ok then
    set_yazi_mode_hint(pane, false)
    show_yazi_toast(window, pane, "manda-toast-yazi-dispatch-failed")
    return
  end
end
...
local MANDA = {
  BLACK = '#15141b',
  -- Render ANSI black foregrounds as light text in MANDA Dark. ANSI black
...
local function tab_display_title(tab, effective_config)
  ...
  if text == '' then
    ...
    local process_title = show_process and active_pane and manda_foreground_process_title(active_pane) or nil
    local path_title = ''
    if tab then
    ...
    text = manda_context_process_title(path_title, process_title) or ''
  end
...
wezterm.on('format-tab-title', function(tab, tabs, panes, effective_config, hover, max_width)
  ...
  local dot_color = is_light and '#AD8301' or MANDA.ORANGE

  local fg_active = MANDA.WHITE
  local fg_inactive_pane = MANDA.GRAY
  ...
  if #own_panes > 1 and tab.tab_title == '' then
    ...
    for _, p in ipairs(own_panes) do
      ...
      local process_title = show_process and manda_foreground_process_title(p) or nil
      if process_title then
        full_text = manda_context_process_title(full_text, process_title) or ''
        short_text = manda_context_process_title(short_text, process_title) or ''
      end
      if full_text == '' then
...
wezterm.on('manda-launch-lazygit', function(window, pane)
  launch_lazygit(window, pane)
end)

wezterm.on('manda-launch-yazi', function(window, pane)
  launch_yazi(window, pane)
end)

wezterm.on('manda-open-remote-files', function(window, pane)
  open_remote_files(window, pane)
end)

wezterm.on('manda-ai-apply-last-fix', function(window, pane)
  pane = resolve_active_pane(window, pane)
  if not pane then
    return
  end
...
wezterm.on('user-var-changed', function(window, pane, name, value)
  ...
  if name == "manda_user_typing" then
    if not pane then
      return
    end
  ...
  if name ~= "manda_last_exit_code" then
    return
  end
  if not pane then
  ...
  if failed_command == "" then
    ai_debug_log("user-var-changed ignored missing manda_last_cmd")
    return
  end
  ...
  pane_state.pending_job_id = nil
  pcall(function()
    window:perform_action(wezterm.action.EmitEvent("manda-toast-ai-analyzing"), pane)
  end)
...
local function is_user_light_theme()
  ...
  -- should track macOS.
  return resolve_appearance_color_scheme() == 'MANDA Light'
end
...
wezterm.on('window-config-reloaded', function(window, pane)
  local overrides = window:get_config_overrides() or {}
  local scheme = resolve_manda_color_scheme(overrides.color_scheme or config.color_scheme)
  local is_light = scheme == 'MANDA Light'
...
-- Optional editor for local file links. MANDA appends path[:line[:column]].
...
-- ===== Close Protection =====
-- Default is 'SmartPrompt': Cmd+Q and the Quit MANDA menu silently quit when
...
config.color_schemes['MANDA Dark'] = {
  -- Background
  foreground = MANDA.WHITE,
  background = MANDA.BLACK,

  -- Cursor
  cursor_bg = MANDA.PURPLE,
  cursor_fg = MANDA.BLACK,
  cursor_border = MANDA.PURPLE,

  -- Selection
  selection_bg = MANDA.PURPLE_FADING,
  selection_fg = MANDA.WHITE,

  -- Normal colors (ANSI 0-7)
  ansi = {
    MANDA.ANSI_BLACK, -- black
    MANDA.RED,     -- red
    MANDA.GREEN,   -- green
    MANDA.ORANGE,  -- yellow
    MANDA.BLUE,    -- blue
    MANDA.PURPLE,  -- magenta
    MANDA.GREEN,   -- cyan
    MANDA.WHITE,   -- white
  },
  ...
  brights = {
    MANDA.GRAY,    -- bright black
    MANDA.RED,     -- bright red
    MANDA.GREEN,   -- bright green
    MANDA.ORANGE,  -- bright yellow
    MANDA.BRIGHT_BLUE, -- bright blue
    MANDA.PURPLE,  -- bright magenta
    MANDA.GREEN,   -- bright cyan
    MANDA.WHITE,   -- bright white
  },

  split = MANDA.SURFACE_ACTIVE,

  -- Tab bar colors
  tab_bar = {
    background = MANDA.BLACK,
    inactive_tab_edge = MANDA.BLACK,

    active_tab = {
      bg_color = MANDA.SURFACE_ACTIVE,
      fg_color = MANDA.WHITE,
      intensity = 'Bold',
      underline = 'None',
      italic = false,
      strikethrough = false,
    ...
    inactive_tab = {
      bg_color = MANDA.BLACK,
      fg_color = MANDA.GRAY,
      intensity = 'Normal',
    },

    inactive_tab_hover = {
      bg_color = MANDA.SURFACE,
      fg_color = MANDA.WHITE,
      italic = false,
    },
  ...
  color_overrides = {
    ...
    [MANDA.WHITE] = '#4A4954',  -- ANSI 7/15 (white) background
  },

  -- Hermes and some Rich/prompt_toolkit surfaces emit black truecolor text
  -- on dark terminals. Keep those foregrounds readable in MANDA Dark.
...
-- ===== MANDA Light Theme =====
config.color_schemes['MANDA Light'] = {
  foreground = '#100F0F',
  background = '#FFFCF0',
  ...
  -- invisible against MANDA Light's cream background.
  foreground_color_overrides = {
...
config.color_schemes['MANDA Theme'] = config.color_schemes['MANDA Dark']
config.color_scheme = resolve_manda_color_scheme(config.color_scheme)
...
get_window_frame_colors = function(scheme)
  scheme = resolve_manda_color_scheme(scheme)
  if scheme == 'MANDA Light' then
    return {
    ...
    return {
      active_titlebar_bg = MANDA.BLACK,
      inactive_titlebar_bg = MANDA.BLACK,
      active_titlebar_fg = MANDA.WHITE,
      inactive_titlebar_fg = MANDA.GRAY,
      active_titlebar_border_bottom = MANDA.BLACK,
      inactive_titlebar_border_bottom = MANDA.BLACK,
      border_left_width = 0,
...
do
  if not user_has_custom_window_frame then
    -- Use the user's intended scheme (scanned from their manda.lua) rather than
...
config.keys = (function() return {
  ...
  -- Cmd+Shift+A: open MANDA AI settings in current pane
  {
    key = 'A',
    mods = 'CMD|SHIFT',
    action = wezterm.action.EmitEvent('run-manda-ai-config'),
  },

  -- Cmd+Shift+E: apply latest MANDA Assistant suggestion for the active pane
  {
    key = 'E',
    mods = 'CMD|SHIFT',
    action = wezterm.action.EmitEvent('manda-ai-apply-last-fix'),
  },
  ...
  -- ~/.config/manda/manda.lua with action = wezterm.action.EmitEvent('manda-ai-chat').
  {
    key = 'l',
    mods = 'CMD',
  ...
  -- Cmd+Shift+G: launch lazygit in current pane
  {
    key = 'G',
    mods = 'CMD|SHIFT',
    action = wezterm.action.EmitEvent('manda-launch-lazygit'),
  },
  ...
  -- Cmd+Shift+Y: launch yazi in current pane
  {
    key = 'Y',
    mods = 'CMD|SHIFT',
    action = wezterm.action.EmitEvent('manda-launch-yazi'),
  },
  ...
  -- Cmd+Shift+R: open the current SSH domain in a local yazi tab via sshfs
  {
    key = 'R',
    mods = 'CMD|SHIFT',
    action = wezterm.action.EmitEvent('manda-open-remote-files'),
  },
...
wezterm.on('gui-startup', function(cmd)
  ...
  local state_file = manda_config_dir .. "/state.json"
  local is_first_run = false
  local needs_update = false

  local function ensure_state_dir()
    os.execute(string.format("mkdir -p %q", manda_config_dir))
  end
  ...
  if not sf and manda_config_dir ~= manda_state_dir then
    -- Pre-XDG releases stored state at ~/.config/manda regardless of
    ...
    sf = io.open(manda_state_dir .. "/state.json", "r")
